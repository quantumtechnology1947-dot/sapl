...
public class Module_Accounts_Transactions_SalesInvoice_New_Details : Page, IRequiresSessionState
{
	...
	protected RegularExpressionValidator RegDateOfRemoval;
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		try
		{
			...
			LblPONo.Text = PN;
			string cmdText2 = fun.select("Id,Description", "tblACC_SalesInvoice_Master_Type", "Id='" + typ + "'");
			...
			if (!base.IsPostBack)
			{
				string cmdText3 = fun.select("InvoiceNo", "tblACC_SalesInvoice_Master", "CompId='" + CompId + "' AND FinYearId='" + FinYearId + "' order by InvoiceNo desc");
				SqlCommand selectCommand3 = new SqlCommand(cmdText3, con);
				SqlDataAdapter sqlDataAdapter3 = new SqlDataAdapter(selectCommand3);
				DataSet dataSet3 = new DataSet();
				sqlDataAdapter3.Fill(dataSet3, "tblACC_SalesInvoice_Master");
				string text2 = "";
				...
				string cmdText4 = fun.select("Id", "tblExciseser_Master", "Live='1'");
				...
				{
					DrpServiceTax.SelectedValue = dataSet4.Tables[0].Rows[0]["Id"].ToString();
				}
				fun.dropdownCountry(DrpByCountry, DrpByState);
				fun.dropdownCountry(DrpCoCountry, DrpCoState);
				string cmdText5 = fun.select("CustomerName+' ['+CustomerId+']' As Customer,MaterialDelAddress,MaterialDelCountry,MaterialDelState,MaterialDelCity,MaterialDelContactNo,MaterialDelFaxNo,ContactPerson,Email,TinVatNo,EccNo,ContactNo,TinCstNo", "SD_Cust_master", "CustomerId='" + CCode + "' And CompId='" + CompId + "'");
				SqlCommand selectCommand5 = new SqlCommand(cmdText5, con);
				SqlDataAdapter sqlDataAdapter5 = new SqlDataAdapter(selectCommand5);
				DataSet dataSet5 = new DataSet();
				sqlDataAdapter5.Fill(dataSet5);
				TxtBYName.Text = dataSet5.Tables[0].Rows[0]["Customer"].ToString();
				TxtByAddress.Text = dataSet5.Tables[0].Rows[0]["MaterialDelAddress"].ToString();
				fun.dropdownCountrybyId(DrpByCountry, DrpByState, "CId='" + dataSet5.Tables[0].Rows[0]["MaterialDelCountry"].ToString() + "'");
				DrpByCountry.SelectedIndex = 0;
				fun.dropdownCountry(DrpByCountry, DrpByState);
				fun.dropdownState(DrpByState, DrpByCity, DrpByCountry);
				fun.dropdownStatebyId(DrpByState, "CId='" + dataSet5.Tables[0].Rows[0]["MaterialDelCountry"].ToString() + "' AND SId='" + dataSet5.Tables[0].Rows[0]["MaterialDelState"].ToString() + "'");
				DrpByState.SelectedValue = dataSet5.Tables[0].Rows[0]["MaterialDelState"].ToString();
				fun.dropdownCity(DrpByCity, DrpByState);
				fun.dropdownCitybyId(DrpByCity, "SId='" + dataSet5.Tables[0].Rows[0]["MaterialDelState"].ToString() + "' AND CityId='" + dataSet5.Tables[0].Rows[0]["MaterialDelCity"].ToString() + "'");
				DrpByCity.SelectedValue = dataSet5.Tables[0].Rows[0]["MaterialDelCity"].ToString();
				TxtByFaxNo.Text = dataSet5.Tables[0].Rows[0]["MaterialDelFaxNo"].ToString();
				TxtByCName.Text = dataSet5.Tables[0].Rows[0]["ContactPerson"].ToString();
				TxtByPhone.Text = dataSet5.Tables[0].Rows[0]["MaterialDelContactNo"].ToString();
				TxtByTINCSTNo.Text = dataSet5.Tables[0].Rows[0]["TinCstNo"].ToString();
				TxtByTINVATNo.Text = dataSet5.Tables[0].Rows[0]["TinVatNo"].ToString();
				TxtByMobile.Text = dataSet5.Tables[0].Rows[0]["ContactNo"].ToString();
				TxtByEmail.Text = dataSet5.Tables[0].Rows[0]["Email"].ToString();
				TxtByECCNo.Text = dataSet5.Tables[0].Rows[0]["EccNo"].ToString();
				fillgrid();
			}
		}
		catch (Exception)
	...
	protected void BtnSubmit_Click(object sender, EventArgs e)
	{
		...
		{
			sqlConnection.Open();
			if (DrpByCountry.SelectedIndex != 0 && DrpByState.SelectedIndex != 0 && DrpByCity.SelectedIndex != 0 && DrpCoCountry.SelectedIndex != 0 && DrpCoState.SelectedIndex != 0 && DrpCoCity.SelectedIndex != 0 && TxtInvNo.Text != "" && TxtDateofIssueInvoice.Text != "" && fun.DateValidation(TxtDateofIssueInvoice.Text) && TxtDateRemoval.Text != "" && fun.DateValidation(TxtDateRemoval.Text) && fun.EmailValidation(TxtByEmail.Text) && fun.EmailValidation(TxtCoEmail.Text) && TxtAdd.Text != "" && fun.NumberValidationQty(TxtAdd.Text) && TxtDeduct.Text != "" && fun.NumberValidationQty(TxtDeduct.Text) && TxtPf.Text != "" && fun.NumberValidationQty(TxtPf.Text) && TxtSed.Text != "" && fun.NumberValidationQty(TxtSed.Text) && TxtAed.Text != "" && fun.NumberValidationQty(TxtAed.Text) && Txtfreight.Text != "" && fun.NumberValidationQty(Txtfreight.Text) && TxtInsurance.Text != "" && fun.NumberValidationQty(TxtInsurance.Text) && DrpCommodity.SelectedValue != "0")
			{
				foreach (GridViewRow row in GridView1.Rows)
				{
				...
				if (num > 0)
				{
					foreach (GridViewRow row2 in GridView1.Rows)
					{
						if (((CheckBox)row2.FindControl("ck")).Checked && ((TextBox)row2.FindControl("TxtReqQty")).Text != "")
						{
							...
							string cmdText2 = fun.select("sum(AmtInPer) As Amt", "tblACC_SalesInvoice_Master,tblACC_SalesInvoice_Details", "tblACC_SalesInvoice_Details.MId=tblACC_SalesInvoice_Master.Id  And  tblACC_SalesInvoice_Master.CompId='" + CompId + "' And tblACC_SalesInvoice_Details.ItemId='" + num14 + "'  Group By tblACC_SalesInvoice_Details.ItemId ");
							SqlCommand selectCommand2 = new SqlCommand(cmdText2, sqlConnection);
							SqlDataAdapter sqlDataAdapter2 = new SqlDataAdapter(selectCommand2);
							DataSet dataSet2 = new DataSet();
							sqlDataAdapter2.Fill(dataSet2);
							...
							string cmdText3 = fun.select("Sum(tblACC_SalesInvoice_Details.ReqQty) as ReqQty", "tblACC_SalesInvoice_Master,tblACC_SalesInvoice_Details", "tblACC_SalesInvoice_Details.MId=tblACC_SalesInvoice_Master.Id  And  tblACC_SalesInvoice_Master.CompId='" + CompId + "' And tblACC_SalesInvoice_Details.ItemId='" + num14 + "'  Group By tblACC_SalesInvoice_Details.ItemId");
							SqlCommand selectCommand3 = new SqlCommand(cmdText3, sqlConnection);
							SqlDataAdapter sqlDataAdapter3 = new SqlDataAdapter(selectCommand3);
							DataSet dataSet3 = new DataSet();
							sqlDataAdapter3.Fill(dataSet3);
							...
							if (num2 > 0)
							{
								string text3 = TimeOfIssue.Hour.ToString("D2") + ":" + TimeOfIssue.Minute.ToString("D2") + ":" + TimeOfIssue.Second.ToString("D2") + " " + TimeOfIssue.AmPm;
								string text4 = TimeOfRemove.Hour.ToString("D2") + ":" + TimeOfRemove.Minute.ToString("D2") + ":" + TimeOfRemove.Second.ToString("D2") + " " + TimeOfRemove.AmPm;
								int num17 = 0;
								int num18 = 0;
								int num19 = 0;
								if (typ == "2")
								{
									num17 = Convert.ToInt32(DrpVAT.SelectedValue);
								}
								else if (typ == "3")
								{
									num19 = Convert.ToInt32(DrpCst.SelectedValue);
									num18 = Convert.ToInt32(DrpVAT.SelectedValue);
								}
								double num20 = Math.Round(Convert.ToDouble(TxtOtherAmt.Text), 2);
								string cmdText4 = fun.insert("tblACC_SalesInvoice_Master", "SysDate,SysTime,CompId,FinYearId, SessionId, InvoiceNo,PONo,WONo,InvoiceMode,DateOfIssueInvoice ,DateOfRemoval,TimeOfIssueInvoice,TimeOfRemoval,NatureOfRemoval,Commodity, ModeOfTransport,RRGCNo,VehiRegNo,DutyRate,  CustomerCode,CustomerCategory,Buyer_name,  Buyer_add,Buyer_country,Buyer_state, Buyer_city,Buyer_cotper,Buyer_ph,Buyer_email,Buyer_ecc,Buyer_tin  ,Buyer_mob  ,Buyer_fax,Buyer_vat,Cong_name ,Cong_add,Cong_Country,Cong_state,Cong_city,Cong_cotper ,  Cong_ph  ,  Cong_email  ,  Cong_ecc  ,Cong_tin,Cong_mob,Cong_fax,Cong_vat,AddType,AddAmt,DeductionType,Deduction,PFType,PF,CENVAT,SED,AED,VAT,SelectedCST,CST,FreightType,Freight,InsuranceType,Insurance ,SEDType,AEDType,POId,OtherAmt", "'" + CDate + "','" + CTime + "','" + CompId + "','" + FinYearId + "','" + sId + "','" + text + "','" + PN + "','" + WN + "','" + typ + "','" + fun.FromDate(TxtDateofIssueInvoice.Text) + "','" + fun.FromDate(TxtDateRemoval.Text) + "','" + text3 + "','" + text4 + "','" + DrpNatureremovable.SelectedValue + "','" + DrpCommodity.SelectedValue + "','" + DrpTransport.SelectedValue + "','" + TxtRRGCNo.Text + "','" + TxtRegistrationNo.Text + "','" + TxtDutyRate.Text + "','" + CCode + "','" + DrpCategory.SelectedValue + "','" + TxtBYName.Text + "','" + TxtByAddress.Text + "','" + DrpByCountry.SelectedValue + "','" + DrpByState.SelectedValue + "','" + DrpByCity.SelectedValue + "','" + TxtByCName.Text + "','" + TxtByPhone.Text + "','" + TxtByEmail.Text + "','" + TxtByECCNo.Text + "','" + TxtByTINCSTNo.Text + "','" + TxtByMobile.Text + "','" + TxtByFaxNo.Text + "','" + TxtByTINVATNo.Text + "','" + TxtCName.Text + "','" + TxtCAddress.Text + "','" + DrpCoCountry.SelectedValue + "','" + DrpCoState.SelectedValue + "','" + DrpCoCity.SelectedValue + "','" + TxtCoPersonName.Text + "','" + TxtCoPhoneNo.Text + "','" + TxtCoEmail.Text + "','" + TxtECoCCNo.Text + "','" + TxtCoTinCSTNo.Text + "','" + TxtCoMobileNo.Text + "','" + TxtCoFaxNo.Text + "','" + TxtCoTinVatNo.Text + "','" + DrpAdd.SelectedValue + "','" + TxtAdd.Text + "','" + DrpDed.SelectedValue + "','" + TxtDeduct.Text + "','" + DrpPAF.SelectedValue + "','" + TxtPf.Text + "','" + DrpServiceTax.SelectedValue + "','" + TxtSed.Text + "','" + TxtAed.Text + "','" + num17 + "','" + num19 + "','" + num18 + "','" + DrpFreight.SelectedValue + "','" + Txtfreight.Text + "','" + DrpInsurance.SelectedValue + "','" + TxtInsurance.Text + "','" + DrpSED.SelectedValue + "','" + DrpAED.SelectedValue + "','" + num13 + "','" + num20 + "'");
								SqlCommand sqlCommand = new SqlCommand(cmdText4, sqlConnection);
								sqlCommand.ExecuteNonQuery();
								string cmdText5 = fun.select("Id", "tblACC_SalesInvoice_Master", " CompId='" + CompId + "' order by Id desc");
								SqlCommand selectCommand4 = new SqlCommand(cmdText5, sqlConnection);
								SqlDataAdapter sqlDataAdapter4 = new SqlDataAdapter(selectCommand4);
								DataSet dataSet4 = new DataSet();
								sqlDataAdapter4.Fill(dataSet4, "tblACC_SalesInvoice_Master");
								text2 = dataSet4.Tables[0].Rows[0]["Id"].ToString();
							}
							double num21 = Convert.ToDouble(decimal.Parse(((Label)row2.FindControl("lblRate")).Text.ToString()).ToString("N2"));
							string selectedValue = ((DropDownList)row2.FindControl("DrpUnitQty")).SelectedValue;
							string cmdText6 = fun.insert("tblACC_SalesInvoice_Details", "InvoiceNo,MId,ItemId,Unit,Qty,ReqQty,AmtInPer,Rate", "'" + text + "','" + text2 + "','" + num14 + "','" + selectedValue + "','" + num10 + "','" + num12 + "','" + num11 + "','" + num21 + "'");
			...
			if (num2 == 0)
			{
				base.Response.Redirect("SalesInvoice_New.aspx?ModId=11&SubModId=51");
			}
		}
		catch (Exception)
		{
