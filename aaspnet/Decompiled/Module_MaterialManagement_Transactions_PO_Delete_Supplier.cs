...
public class Module_MaterialManagement_Transactions_PO_Delete_Supplier : Page, IRequiresSessionState
{
	...
	public void LoadData(string supcode)
	{
		...
		try
		{
			...
			for (int i = 0; i < dataSet.Tables[0].Rows.Count; i++)
			{
				...
				string cmdText2 = fun.select("*", "tblMM_PO_Master", "SupplierId='" + dataSet.Tables[0].Rows[i]["SupplierId"].ToString() + "' AND FinYearId<='" + FyId + "' AND CompId='" + CompId + "'");
	...
	protected void GridView5_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		try
		{
			if (e.CommandName == "sel")
			{
				...
				base.Response.Redirect("PO_Delete.aspx?Code=" + text + "&ModId=6&SubModId=35");
			}
		}
		catch (Exception)
		{
