...
public class Module_Design_Transactions_BOM_Design_Copy_Tree : Page, IRequiresSessionState
{
	...
	public DataTable GetDataTable()
	{
		...
		{
			sqlConnection.Open();
			string cmdText = fun.select(" ItemId,WONo,PId,CId,Qty", "tblDG_BOM_Master", "WONo='" + wonosrc + "' And CompId='" + CompId + "' And FinYearId<='" + FinYearId + "' Order By PId ASC");
			...
			for (int i = 0; i < dataSet.Tables[0].Rows.Count; i++)
			{
				...
				dataRow[4] = fun.GetItemCode_PartNo(CompId, Convert.ToInt32(dataSet2.Tables[0].Rows[0]["Id"].ToString()));
	...
	protected void btnCopy_Click(object sender, EventArgs e)
	{
		try
		{
			...
			if (cid == num)
			{
				base.Response.Redirect("BOM_Design_CopyWo.aspx?WONoSrc=" + wonosrc + "&WONoDest=" + wonodest + "&DestPId=" + pid + "&DestCId=" + cid + "&ModId=3&SubModId=23&msg=Selection of Assly/Item is incorrect.");
			}
			else
			{
				fun.getBOMnode(num, wonosrc, wonodest, CompId, SessionId, FinYearId, pid, cid);
				base.Response.Redirect("BOM_Design_CopyWo.aspx?WONoSrc=" + wonosrc + "&WONoDest=" + wonodest + "&DestPId=" + pid + "&DestCId=" + cid + "&ModId=3&SubModId=23&msg=BOM is Copied sucessfully in WONo:" + wonodest + " from WONo:" + wonosrc);
			}
		}
		catch (Exception)
		{
	...
	{
		base.Response.Redirect("BOM_Design_CopyWo.aspx?WONoSrc=" + wonosrc + "&WONoDest=" + wonodest + "&DestPId=" + pid + "&DestCId=" + cid);
	}
