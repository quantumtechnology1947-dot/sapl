...
public class Module_MaterialManagement_Transactions_PR_Delete : Page, IRequiresSessionState
{
	...
	public void makegrid(string prno, string empid)
	{
		try
		{
			...
			string cmdText = fun.select("Id,SessionId,FinYearId,PRNo,REPLACE(CONVERT(varchar, CONVERT(datetime, SUBSTRING(tblMM_PR_Master.SysDate, CHARINDEX('-', tblMM_PR_Master.SysDate) + 1, 2) + '-' + LEFT(tblMM_PR_Master.SysDate,CHARINDEX('-', tblMM_PR_Master.SysDate) - 1) + '-' + RIGHT(tblMM_PR_Master.SysDate, CHARINDEX('-', REVERSE(tblMM_PR_Master.SysDate)) - 1)), 103), '/', '-') AS SysDate,SysTime AS Time", "tblMM_PR_Master", "  FinYearId<='" + FyId + "' AND CompId='" + CompId + "'" + text + text2 + "Order by Id Desc");
	...
	protected void drpfield_SelectedIndexChanged(object sender, EventArgs e)
	{
		...
		else if (drpfield.SelectedValue == "0")
		{
