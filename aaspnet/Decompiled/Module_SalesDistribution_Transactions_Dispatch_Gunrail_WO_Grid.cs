...
public class Module_SalesDistribution_Transactions_Dispatch_Gunrail_WO_Grid : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		try
		{
			...
			string cmdText2 = fun.delete("tblDG_Gunrail_CrossRail_Dispatch_Temp", "CompId='" + CompId + "' AND SessionId='" + sId + "'");
			SqlCommand sqlCommand = new SqlCommand(cmdText2, sqlConnection);
			sqlConnection.Open();
			sqlCommand.ExecuteNonQuery();
			sqlConnection.Close();
			string cmdText3 = fun.delete("tblDG_Gunrail_LongRail_Dispatch_Temp", "CompId='" + CompId + "' AND SessionId='" + sId + "'");
	...
	{
		base.Response.Redirect("Dashboard_BOM.aspx?ModId=3&SubModId=26");
	}
	...
	public void BindDataCust(string Cid, string EID, int C)
	{
		try
		{
			...
			string value4 = " And SD_Cust_WorkOrder_Master.WONo not in (select WONo from tblDG_BOM_Master)";
