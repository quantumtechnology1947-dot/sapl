...
public class Module_MaterialManagement_Transactions_SPR_Approve : Page, IRequiresSessionState
{
	...
	private string parentPage = "SPR_Approve.aspx";
	...
	public void makegrid(string sprno, string empid)
	{
		try
		{
			...
			string cmdText = fun.select("*", "tblMM_SPR_Master", "FinYearId<='" + FyId + "' And  CompId='" + CompId + "' AND Checked='1'" + text + text2 + " AND Approve='0' Order by Id Desc");
		...
		try
		{
			...
			foreach (GridViewRow row in GridView2.Rows)
			{
				...
				if (checkBox.Checked)
				{
					...
					string cmdText = fun.update("tblMM_SPR_Master", "Approve='1',ApprovedBy='" + sId + "',ApproveDate='" + currDate + "',ApproveTime='" + currTime + "'", "CompId='" + CompId + "' AND SPRNo='" + text + "'  AND Id='" + text2 + "'");
