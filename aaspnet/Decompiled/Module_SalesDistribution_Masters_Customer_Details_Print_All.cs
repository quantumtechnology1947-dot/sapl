...
public class Module_SalesDistribution_Masters_Customer_Details_Print_All : Page, IRequiresSessionState
{
	...
	protected void Page_Init(object sender, EventArgs e)
	{
		...
		try
		{
			if (!base.IsPostBack)
			{
				...
				string cmdText = fun.select("*", "SD_Cust_master", " CompId='" + cId + "'");
				SqlCommand selectCommand = new SqlCommand(cmdText, sqlConnection);
				SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(selectCommand);
				DataSet dataSet = new DataSet();
				sqlDataAdapter.Fill(dataSet, "SD_Cust_master");
				dataTable.Columns.Add(new DataColumn("SalesId", typeof(int)));
				...
				for (int i = 0; i < dataSet.Tables[0].Rows.Count; i++)
				{
					...
					dataRow[0] = Convert.ToInt32(dataSet.Tables[0].Rows[i]["SalesId"]);
					dataRow[1] = Convert.ToInt32(dataSet.Tables[0].Rows[i]["CompId"]);
					dataRow[2] = dataSet.Tables[0].Rows[i]["CustomerName"].ToString();
					...
					dataRow[6] = dataSet.Tables[0].Rows[i]["RegdContactNo"].ToString();
					dataRow[7] = dataSet.Tables[0].Rows[i]["Email"].ToString();
					dataTable.Rows.Add(dataRow);
					dataTable.AcceptChanges();
				}
				DataSet dataSet5 = new AllCustomerPrint();
				dataSet5.Tables[0].Merge(dataTable);
				dataSet5.AcceptChanges();
				report = new ReportDocument();
				report.Load(base.Server.MapPath("~/Module/SalesDistribution/Masters/Reports/CustomerPrintReportAll.rpt"));
