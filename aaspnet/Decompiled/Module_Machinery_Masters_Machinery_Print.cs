...
public class Module_Machinery_Masters_Machinery_Print : Page, IRequiresSessionState
{
	...
	public void fillGrid(string sql, GridView GridView2)
	{
		...
		try
		{
			...
			for (int i = 0; i < dataSet.Tables[0].Rows.Count; i++)
			{
				...
				string cmdText2 = fun.select("Location,FileName,Id", " tblMS_Master ", string.Concat(" ItemId='", dataSet.Tables[0].Rows[i]["Id"], "'"));
				...
				if (dataSet3.Tables[0].Rows.Count > 0)
				{
					...
					dataRow[7] = dataSet3.Tables[0].Rows[0]["FileName"].ToString();
					dataRow[6] = dataSet3.Tables[0].Rows[0]["Id"].ToString();
				}
	...
	protected void GridView2_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		try
		{
			...
			if (e.CommandName == "Sel" && num != 0)
			{
				string randomAlphaNumeric = fun.GetRandomAlphaNumeric();
				base.Response.Redirect("~/Module/Machinery/Masters/Machinery_Print_Details.aspx?Id=" + num + "&Key=" + randomAlphaNumeric + "&ModId=15&SubModId=67");
			}
			if (e.CommandName == "Download")
			{
				string cmdText = fun.select("*", "tblMS_Master", "CompId='" + CompId + "' AND Id='" + num2 + "'");
				SqlCommand selectCommand = new SqlCommand(cmdText, con);
				SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(selectCommand);
				DataSet dataSet = new DataSet();
				sqlDataAdapter.Fill(dataSet);
				if (dataSet.Tables[0].Rows[0]["FileName"] == DBNull.Value || dataSet.Tables[0].Rows[0]["FileName"].ToString() == "")
				{
				...
				base.Response.Redirect("~/Controls/DownloadFile.aspx?Id=" + dataSet.Tables[0].Rows[0]["Id"].ToString() + "&tbl=tblMS_Master&qfd=FileData&qfn=fileName&qct=ContentType");
			}
		}
		catch (Exception)
		{
