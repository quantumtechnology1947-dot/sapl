...
public class Module_Design_Transactions_TPL_Design_PrintWo : Page, IRequiresSessionState
{
		...
		try
		{
			if (e.CommandName == "Sel")
			{
				...
				if (((TextBox)gridViewRow.FindControl("TxtBOMDate")).Text != "" && ((TextBox)gridViewRow.FindControl("TxtUptoDate")).Text != "")
				{
					...
					base.Response.Redirect("TPL_Design_Print_Tree.aspx?WONo=" + text + "&SD=" + text2 + "&TD=" + text3 + "&ModId=3&SubModId=23");
				}
			}
		}
		catch (Exception)
	...
	public void BindDataCust(string Cid, string EID)
	{
		try
		{
			...
			string cmdText = fun.select("SD_Cust_WorkOrder_Master.EnqId,SD_Cust_WorkOrder_Master.PONo,SD_Cust_WorkOrder_Master.Id,SD_Cust_WorkOrder_Master.CustomerId,SD_Cust_WorkOrder_Master.WONo,SD_Cust_WorkOrder_Master.SessionId,SD_Cust_WorkOrder_Master.FinYearId, SD_Cust_WorkOrder_Master.SysDate  As WODate ", "SD_Cust_WorkOrder_Master", "SD_Cust_WorkOrder_Master.FinYearId<='" + FinYearId + "'And SD_Cust_WorkOrder_Master.CompId='" + CompId + "'" + text + text2 + "Order by Id Desc");
			...
			dataTable.Columns.Add(new DataColumn("TPLDate", typeof(string)));
			...
			dataTable.Columns.Add(new DataColumn("EnqId", typeof(int)));
			...
			for (int i = 0; i < dataSet.Tables[0].Rows.Count; i++)
			{
				...
				{
					string cmdText2 = fun.select("SysDate As TPLDate", "tblDG_TPL_Master", " FinYearId<='" + FinYearId + "'AND CompId='" + CompId + "'  AND  WONo='" + dataSet.Tables[0].Rows[i]["WONo"].ToString() + "'  order by SysDate Asc");
					SqlCommand selectCommand2 = new SqlCommand(cmdText2, sqlConnection);
					SqlDataAdapter sqlDataAdapter2 = new SqlDataAdapter(selectCommand2);
					DataSet dataSet2 = new DataSet();
					sqlDataAdapter2.Fill(dataSet2);
					if (dataSet2.Tables[0].Rows.Count > 0 && dataSet2.Tables[0].Rows[0]["TPLDate"] != DBNull.Value)
					{
						dataRow[3] = fun.FromDateDMY(dataSet2.Tables[0].Rows[0]["TPLDate"].ToString());
					}
					...
					dataRow[9] = dataSet.Tables[0].Rows[i]["EnqId"].ToString();
					dataRow[10] = dataSet.Tables[0].Rows[i]["PONo"].ToString();
