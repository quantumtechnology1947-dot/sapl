...
public class Module_Accounts_Transactions_ProformaInvoice_New : Page, IRequiresSessionState
{
		...
		try
		{
			...
			if (dataSet.Tables[0].Rows.Count > 0)
			{
				for (int i = 0; i < dataSet.Tables[0].Rows.Count; i++)
				{
					...
					if (num > 0)
					{
						string cmdText6 = fun.select("count(*) as Rcount", "tblACC_ProformaInvoice_Master", "CompId='" + CompId + "' AND POId='" + dataSet.Tables[0].Rows[i]["POId"].ToString() + "'");
						...
						dataRow[6] = dataSet6.Tables[0].Rows[0]["Rcount"].ToString();
	...
	protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		try
		{
			if (e.CommandName == "Sel")
			{
				...
				string selectedValue = ((DropDownList)gridViewRow.FindControl("drp1")).SelectedValue;
				string text5 = ((Label)gridViewRow.FindControl("lblCustId")).Text;
				if (text4 != "")
				{
					base.Response.Redirect("ProformaInvoice_New_Details.aspx?poid=" + base.Server.UrlEncode(fun.Encrypt(text2)) + "&wn=" + base.Server.UrlEncode(fun.Encrypt(text4)) + "&pn=" + base.Server.UrlEncode(fun.Encrypt(text)) + "&date=" + base.Server.UrlEncode(fun.Encrypt(text3)) + "&ty=" + base.Server.UrlEncode(fun.Encrypt(selectedValue.ToString())) + "&cid=" + base.Server.UrlEncode(fun.Encrypt(text5)) + "&ModId=11&SubModId=104");
				}
				else
				{
					string empty = string.Empty;
					empty = "Select WONo and Against.";
