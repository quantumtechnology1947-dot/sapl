...
public class Module_SalesDistribution_Transactions_CustPO_Delete : Page, IRequiresSessionState
{
	...
	public void BindDataCust(string Cid, string EID)
	{
		try
		{
			...
			for (int i = 0; i < dataSet.Tables[0].Rows.Count; i++)
			{
				...
				string cmdText2 = fun.select("CloseOpen", "SD_Cust_WorkOrder_Master", "FinYearId<='" + num + "' AND CompId='" + num2 + "'  And PONo='" + dataSet.Tables[0].Rows[i]["PONo"].ToString() + "'");
				...
				if (num3 == 0)
				{
					string cmdText3 = fun.select("CustomerName,CustomerId", "SD_Cust_Master", "CompId='" + num2 + "'  AND  CustomerId='" + dataSet.Tables[0].Rows[i]["CustomerId"].ToString() + "'");
					...
					string cmdText5 = fun.select("Title+'.'+EmployeeName As EmpLoyeeName", "tblHR_OfficeStaff", string.Concat("CompId='", num2, "' AND EmpId='", dataSet.Tables[0].Rows[i]["SessionId"], "'"));
