...
public class Module_SalesDistribution_Transactions_CustPO_Edit : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		try
		{
			...
			string cmdText = fun.delete("SD_Cust_PO_Details_Temp", "SessionId='" + sId + "' and CompId='" + CompId + "'");
	...
	public void BindDataCust(string Cid, string EID)
	{
		try
		{
			...
			string cmdText = fun.select("PONo,POId,EnqId,CustomerId,SessionId,FinYearId,REPLACE(CONVERT(varchar, CONVERT(datetime, SUBSTRING(SD_Cust_PO_Master.SysDate, CHARINDEX('-', SD_Cust_PO_Master.SysDate) + 1, 2) + '-' + LEFT(SD_Cust_PO_Master.SysDate,CHARINDEX('-', SD_Cust_PO_Master.SysDate) - 1) + '-' + RIGHT(SD_Cust_PO_Master.SysDate, CHARINDEX('-', REVERSE(SD_Cust_PO_Master.SysDate)) - 1)), 103), '/', '-')AS SysDate", "SD_Cust_PO_Master", "FinYearId<='" + FinYearId + "' AND CompId='" + CompId + "'" + text + text2 + "Order by POId Desc");
