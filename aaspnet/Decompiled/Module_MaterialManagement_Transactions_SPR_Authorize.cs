...
public class Module_MaterialManagement_Transactions_SPR_Authorize : Page, IRequiresSessionState
{
	...
	private string parentPage = "SPR_Authorize.aspx";
	...
	public void makegrid(string sprno, string empid)
	{
		try
		{
			...
			string cmdText = fun.select("*", "tblMM_SPR_Master", "  FinYearId<='" + FyId + "' And   CompId='" + CompId + "' AND Approve='1'" + text + text2 + " AND Authorize='0' Order by Id Desc");
		...
		try
		{
			...
			foreach (GridViewRow row in GridView2.Rows)
			{
				...
				if (checkBox.Checked)
				{
					...
					string cmdText = fun.update("tblMM_SPR_Master", "Authorize='1',AuthorizedBy='" + sId + "',AuthorizeDate='" + CDate + "',AuthorizeTime='" + CTime + "'", "CompId='" + CompId + "' AND SPRNo='" + text + "'  AND Id='" + text2 + "'");
