...
public class Module_HR_Transactions_OfferLetter_Delete : Page, IRequiresSessionState
{
	...
	public void binddata(string EmpName)
	{
		...
		try
		{
			...
			foreach (GridViewRow row in GridView2.Rows)
			{
				...
				string cmdText7 = fun.select("*", "tblHR_OfficeStaff", "OfferId='" + text2 + "'");
				SqlCommand selectCommand7 = new SqlCommand(cmdText7, sqlConnection);
				SqlDataAdapter sqlDataAdapter7 = new SqlDataAdapter(selectCommand7);
				DataSet dataSet7 = new DataSet();
				sqlDataAdapter7.Fill(dataSet7, "tblHR_OfficeStaff");
	...
	protected void GridView2_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		try
		{
			if (e.CommandName == "Delete")
			{
				...
				string cmdText = fun.select("*", "tblHR_OfficeStaff", "OfferId='" + text + "'");
				SqlCommand selectCommand = new SqlCommand(cmdText, sqlConnection);
				SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(selectCommand);
				DataSet dataSet = new DataSet();
				sqlDataAdapter.Fill(dataSet, "tblHR_OfficeStaff");
				if (dataSet.Tables[0].Rows.Count == 0)
				{
					string cmdText2 = fun.delete("tblHR_Offer_Master", "OfferId='" + text + "'");
					SqlCommand sqlCommand = new SqlCommand(cmdText2, sqlConnection);
					sqlCommand.ExecuteNonQuery();
					string cmdText3 = fun.delete("tblHR_Offer_Accessories", "MId='" + text + "'");
