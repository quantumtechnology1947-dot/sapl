...
public class Module_Accounts_Transactions_Credit_Note : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		try
		{
			...
			string text = "";
			string cmdText = fun.select("*", "tblACC_CreditNote", "CompId='" + Cid + "' AND FinYearId<='" + Fyid + "' Order by Id Desc");
	...
	public void fillgrid()
	{
		try
		{
			...
			dataTable.Columns.Add(new DataColumn("CreditNo", typeof(string)));
			...
			for (int i = 0; i < dataSet.Tables[0].Rows.Count; i++)
			{
				...
				dataRow[2] = dataSet.Tables[0].Rows[i]["CreditNo"].ToString();
	...
	protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		try
		{
			...
			if (e.CommandName == "Add")
			{
				...
				string cmdText = fun.select("CreditNo", "tblACC_CreditNote", "CompId='" + Cid + "' AND FinYearId='" + Fyid + "' order by CreditNo desc");
				SqlCommand selectCommand = new SqlCommand(cmdText, con);
				SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(selectCommand);
				DataSet dataSet = new DataSet();
				sqlDataAdapter.Fill(dataSet, "tblACC_CreditNote");
				string text5 = "";
				text5 = ((dataSet.Tables[0].Rows.Count <= 0) ? "0001" : (Convert.ToInt32(dataSet.Tables[0].Rows[0]["CreditNo"]) + 1).ToString("D4"));
				...
				{
					string cmdText2 = fun.insert("tblACC_CreditNote", "SysDate,SysTime,CompId,SessionId,FinYearId,Date,CreditNo,SCE,Amount,Refrence,Particulars,Types", "'" + currDate + "','" + currTime + "','" + Cid + "','" + Sid + "','" + Fyid + "','" + text + "','" + text5 + "','" + code + "','" + Convert.ToDouble(decimal.Parse(text4).ToString("N2")) + "','" + text2 + "','" + text3 + "','" + num + "'");
			...
			if (e.CommandName == "Add1")
			{
				...
				string cmdText3 = fun.select("CreditNo", "tblACC_CreditNote", "CompId='" + Cid + "' AND FinYearId='" + Fyid + "' order by CreditNo desc");
				SqlCommand selectCommand2 = new SqlCommand(cmdText3, con);
				SqlDataAdapter sqlDataAdapter2 = new SqlDataAdapter(selectCommand2);
				DataSet dataSet2 = new DataSet();
				sqlDataAdapter2.Fill(dataSet2);
				string text10 = "";
				text10 = ((dataSet2.Tables[0].Rows.Count <= 0) ? "0001" : (Convert.ToInt32(dataSet2.Tables[0].Rows[0]["CreditNo"]) + 1).ToString("D4"));
				...
				{
					string cmdText4 = fun.insert("tblACC_CreditNote", "SysDate,SysTime,CompId,SessionId,FinYearId,Date,CreditNo,SCE,Amount,Refrence,Particulars,Types", "'" + currDate + "','" + currTime + "','" + Cid + "','" + Sid + "','" + Fyid + "','" + text6 + "','" + text10 + "','" + code2 + "','" + Convert.ToDouble(decimal.Parse(text9).ToString("N2")) + "','" + text7 + "','" + text8 + "','" + num3 + "'");
	...
	protected void GridView1_RowUpdating(object sender, GridViewUpdateEventArgs e)
	{
		try
		{
			...
			{
				string cmdText = fun.update("tblACC_CreditNote", "Date='" + text + "',SCE='" + code + "',Amount='" + Convert.ToDouble(decimal.Parse(text4).ToString("N2")) + "',Refrence='" + text2 + "',Particulars='" + text3 + "',Types='" + num2 + "'", "Id=" + num + " And CompId='" + Cid + "'");
	...
	protected void GridView1_RowDeleting(object sender, GridViewDeleteEventArgs e)
	{
		...
		{
			int num = Convert.ToInt32(GridView1.DataKeys[e.RowIndex].Value);
			SqlCommand sqlCommand = new SqlCommand(fun.delete("tblACC_CreditNote", "Id='" + num + "' And CompId='" + Cid + "'"), con);
			con.Open();
			sqlCommand.ExecuteNonQuery();
			con.Close();
			Page.Response.Redirect(Page.Request.Url.ToString(), endResponse: true);
	...
	public static string[] sql(string prefixText, int count, string contextKey)
	{
		...
		switch (HttpContext.Current.Session["Val"].ToString())
		{
		case "1":
	...
	public static string[] sql2(string prefixText, int count, string contextKey)
	{
		...
		sqlConnection.Open();
		string selectCommandText = "";
		switch (HttpContext.Current.Session["ValE"].ToString())
		{
		case "1":
			selectCommandText = clsFunctions2.select("EmpId,EmployeeName", "tblHR_OfficeStaff", "CompId='" + num + "' Order by EmployeeName");
			break;
