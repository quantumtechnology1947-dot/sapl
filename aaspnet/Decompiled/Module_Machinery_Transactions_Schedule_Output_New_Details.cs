...
public class Module_Machinery_Transactions_Schedule_Output_New_Details : Page, IRequiresSessionState
{
	...
	public void FillTempGrid2()
	{
		try
		{
			string cmdText = fun.select1("*", "tblMS_JobCompletion_temp Order By Id Desc");
	...
	public void PagePreLoad()
	{
		try
		{
			...
			dataTable.Columns.Add(new DataColumn("Id", typeof(int)));
			string cmdText = fun.select1("Symbol,Id", "Unit_Master");
			SqlCommand selectCommand = new SqlCommand(cmdText, con);
			SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(selectCommand);
			DataSet dataSet2 = new DataSet();
			...
			if (dataSet2.Tables[0].Rows.Count > 0)
			{
				...
				{
					DataRow dataRow = dataTable.NewRow();
					dataRow[0] = dataSet2.Tables[0].Rows[i]["Symbol"].ToString();
					dataRow[1] = Convert.ToInt32(dataSet2.Tables[0].Rows[i]["Id"]);
					dataTable.Rows.Add(dataRow);
					dataTable.AcceptChanges();
				}
			}
			...
			{
				((DropDownList)row.FindControl("DrpUnit")).DataSource = dataSet.Tables[0];
				((DropDownList)row.FindControl("DrpUnit")).DataTextField = "Symbol";
				((DropDownList)row.FindControl("DrpUnit")).DataValueField = "Id";
				((DropDownList)row.FindControl("DrpUnit")).DataBind();
				((DropDownList)row.FindControl("DrpUnit")).Items.Insert(0, "Select");
			}
		}
		catch (Exception)
		{
	...
	public void FillTempGrid()
	{
		try
		{
			...
			for (int i = 0; i < dataSet.Tables[0].Rows.Count; i++)
			{
				...
				int num2 = 0;
				if (Convert.ToInt32(dataSet.Tables[0].Rows[i]["Shift"]) == 0)
				{
					dataRow[3] = "Day";
				}
				else
				...
				double num3 = 0.0;
				num3 = Convert.ToDouble(dataSet.Tables[0].Rows[i]["Qty"]);
				dataRow[4] = num3;
				...
				dataRow[9] = Convert.ToInt32(dataSet.Tables[0].Rows[i]["MId"]);
				string selectCommandText = fun.select("EmployeeName As Operator", "tblHR_OfficeStaff", "CompId='" + CompId + "' And EmpId='" + dataSet.Tables[0].Rows[i]["Operator"].ToString() + "'");
				SqlDataAdapter sqlDataAdapter4 = new SqlDataAdapter(selectCommandText, con);
				DataSet dataSet4 = new DataSet();
				sqlDataAdapter4.Fill(dataSet4, "tblHR_OfficeStaff");
				if (dataSet4.Tables[0].Rows.Count > 0)
				{
					dataRow[10] = dataSet4.Tables[0].Rows[0]["Operator"].ToString();
				}
				dataRow[11] = Convert.ToInt32(dataSet.Tables[0].Rows[i]["Id"]);
				if (!string.IsNullOrEmpty(dataSet.Tables[0].Rows[i]["BatchNo"].ToString()))
				{
	...
	protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		try
		{
			if (e.CommandName == "Add")
			{
				...
				if (((TextBox)gridViewRow.FindControl("TxtoutputQty")).Text != "" && ((DropDownList)gridViewRow.FindControl("DrpUnit")).SelectedItem.Text != "Select")
				{
					int num = Convert.ToInt32(((Label)gridViewRow.FindControl("lblMId")).Text);
					int num2 = Convert.ToInt32(((Label)gridViewRow.FindControl("lblId")).Text);
					int num3 = Convert.ToInt32(((DropDownList)gridViewRow.FindControl("drpunit")).SelectedValue);
					double num4 = Convert.ToDouble(((TextBox)gridViewRow.FindControl("TxtoutputQty")).Text);
					string cmdText = fun.insert("tblMS_JobCompletion_Temp", "MId,DId,OutputQty,UOM", "'" + num + "','" + num2 + "','" + num4 + "','" + num3 + "'");
				...
				else if (((TextBox)gridViewRow.FindControl("TxtoutputQty")).Text == "")
				{
					string empty = string.Empty;
					empty = "Fill Output Qty and Select UOM !";
	...
	{
		base.Response.Redirect("~/Module/Machinery/Transactions/Schedule_Output_New.aspx?ModId=15&SubModId=70");
	}
	...
	protected void GridView2_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		try
		{
			if (e.CommandName == "Del")
			{
				...
				string cmdText = fun.delete("tblMS_JobCompletion_Temp", "Id='" + num + "'");
				SqlCommand sqlCommand = new SqlCommand(cmdText, con);
				con.Open();
				sqlCommand.ExecuteNonQuery();
				con.Close();
	...
	{
		int num = 0;
		string cmdText = fun.select1("*", "tblMS_JobCompletion_temp");
		SqlCommand selectCommand = new SqlCommand(cmdText, con);
		SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(selectCommand);
		DataSet dataSet = new DataSet();
		sqlDataAdapter.Fill(dataSet);
		for (int i = 0; i < dataSet.Tables[0].Rows.Count; i++)
		{
			string cmdText2 = fun.insert("tblMS_JobCompletion", "MId,DId,OutputQty,UOM", string.Concat("'", dataSet.Tables[0].Rows[i]["MId"], "','", dataSet.Tables[0].Rows[i]["DId"], "','", Convert.ToDouble(dataSet.Tables[0].Rows[i]["OutputQty"]), "','", dataSet.Tables[0].Rows[i]["UOM"].ToString(), "'"));
		...
		if (num > 0)
		{
			string cmdText3 = "delete tblMS_JobCompletion_Temp";
