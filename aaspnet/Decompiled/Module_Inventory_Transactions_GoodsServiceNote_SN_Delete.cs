...
public class Module_Inventory_Transactions_GoodsServiceNote_SN_Delete : Page, IRequiresSessionState
{
		...
		try
		{
			...
			for (int i = 0; i < dataSet.Tables[0].Rows.Count; i++)
			{
				...
				if (dataSet4.Tables[0].Rows.Count > 0)
				{
					...
					dataRow[10] = Convert.ToInt32(dataSet.Tables[0].Rows[i]["Id"]);
	...
	protected void GridView2_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		try
		{
			if (e.CommandName == "Sel")
			{
				...
				base.Response.Redirect("GoodsServiceNote_SN_Delete_Details.aspx?Id=" + text7 + "&SupId=" + text + "&GSNNo=" + text2 + "&GINNo=" + text3 + "&GINId=" + text6 + "&PONo=" + text4 + "&FyId=" + text5 + "&ModId=9&SubModId=39");
			}
		}
		catch (Exception)
		{
