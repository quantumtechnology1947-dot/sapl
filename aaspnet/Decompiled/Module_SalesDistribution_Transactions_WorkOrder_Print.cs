...
public class Module_SalesDistribution_Transactions_WorkOrder_Print : Page, IRequiresSessionState
{
	...
	public void BindDataCust(string Cid, string EID, int C)
	{
		try
		{
			...
			dataTable.Columns.Add(new DataColumn("CloseOpen", typeof(string)));
			for (int i = 0; i < dataSet.Tables[0].Rows.Count; i++)
			{
				...
				if (dataSet.Tables[0].Rows.Count > 0)
				{
					...
					dataRow[8] = dataSet.Tables[0].Rows[i]["POId"].ToString();
					dataRow[9] = dataSet.Tables[0].Rows[i]["Id"].ToString();
					int num = 0;
					if (Convert.ToInt32(dataSet.Tables[0].Rows[i]["CloseOpen"]) == 0)
					{
						dataRow[10] = "Open";
					}
					else
	...
	protected void SearchGridView1_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		if (e.CommandName == "sel")
		{
			...
			string text3 = ((Label)gridViewRow.FindControl("lblPONo")).Text;
			string text4 = ((Label)gridViewRow.FindControl("lblPOId")).Text;
			string text5 = ((Label)gridViewRow.FindControl("lblId")).Text;
			string randomAlphaNumeric = fun.GetRandomAlphaNumeric();
			base.Response.Redirect("~/Module/SalesDistribution/Transactions/WorkOrder_Print_Details.aspx?CustomerId=" + text + "&EnqId=" + text2 + "&PONo=" + text3 + "&POId=" + text4 + "&Id=" + text5 + "&Key=" + randomAlphaNumeric + "&ModId=2&SubModId=13");
		}
	}
}
