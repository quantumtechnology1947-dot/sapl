...
public class Module_Design_Transactions_BOM_Design_WO_TreeView : Page, IRequiresSessionState
{
		...
		try
		{
			...
			if (drpValue == 1)
			{
				...
				for (int j = 0; j < list.Count; j++)
				{
					...
					empty2 = fun.select("ItemId,ItemCode,UOMBasic,FileName,AttName,tblDG_Item_Master.PartNo,ManfDesc,WONo,PId,tblDG_BOM_Master.CId,Qty,tblDG_BOM_Master.Revision", "tblDG_Item_Master,tblDG_BOM_Master", "tblDG_BOM_Master.ItemId='" + Convert.ToInt32(list[j]) + "' And tblDG_Item_Master.Id=tblDG_BOM_Master.ItemId  And tblDG_BOM_Master.CompId='" + CompId + "'And tblDG_BOM_Master.FinYearId<='" + FinYearId + "' And WONo='" + woQueryStr + "'");
					...
					if (dataSet2.Tables[0].Rows.Count > 0)
					{
						...
						dataRow[13] = dataSet2.Tables[0].Rows[0]["Revision"];
			...
			else
			{
				...
				for (int l = 0; l < dataSet.Tables[0].Rows.Count; l++)
				{
					...
					dataRow[13] = dataSet.Tables[0].Rows[l]["Revision"];
					list3.Clear();
					dataTable.Rows.Add(dataRow);
				}
			}
	...
	protected void RadTreeList1_ItemCommand(object sender, TreeListCommandEventArgs e)
	{
		...
		if (e.CommandName == "Sel")
		{
			...
			string cmdText = fun.select("tblDG_Item_Master.CId", "tblDG_Item_Master,tblDG_BOM_Master", "tblDG_Item_Master.Id='" + num3 + "' And tblDG_Item_Master.CId is Null  And tblDG_Item_Master.CompId='" + CompId + "'And tblDG_Item_Master.FinYearId<='" + FinYearId + "'");
			...
			{
				base.Response.Redirect("BOM_WoItems.aspx?WONo=" + text + "&ItemId=" + num3 + "&PId=" + num + "&CId=" + num2 + "&ModId=3&SubModId=26");
			}
			else
			{
				string empty = string.Empty;
		...
		if (e.CommandName == "uploadImg")
		{
			...
			string text2 = ((Label)treeListDataItem3.FindControl("lblWONo")).Text;
			int num5 = Convert.ToInt32(((Label)treeListDataItem3.FindControl("lblItemId")).Text);
			base.Response.Redirect("~/Module/Design/Transactions/BOM_UploadDrw.aspx?WONo=" + text2 + "&Id=" + num5 + "&img=0");
		}
		if (e.CommandName == "downloadSpec")
		{
			...
			base.Response.Redirect("~/Controls/DownloadFile.aspx?Id=" + num6 + "&tbl=tblDG_Item_Master&qfd=AttData&qfn=AttName&qct=AttContentType");
		}
		if (e.CommandName == "uploadSpec")
		{
			TreeListDataItem treeListDataItem5 = e.Item as TreeListDataItem;
			string text3 = ((Label)treeListDataItem5.FindControl("lblWONo")).Text;
			int num7 = Convert.ToInt32(((Label)treeListDataItem5.FindControl("lblItemId")).Text);
			base.Response.Redirect("~/Module/Design/Transactions/BOM_UploadDrw.aspx?WONo=" + text3 + "&Id=" + num7 + "&img=1");
		}
	}
	...
	protected void btnAssembly_Click(object sender, EventArgs e)
	{
		...
		string text2 = "BOM_Design_WO_TreeView.aspx";
		base.Response.Redirect("~/Module/Design/Transactions/BOM_Design_Assembly_New.aspx?WONo=" + text + "&PgUrl=" + text2 + "&ModId=3&SubModId=26");
	}
	...
	protected void RadTreeList1_AutoGeneratedColumnCreated(object sender, TreeListAutoGeneratedColumnCreatedEventArgs e)
	{
		...
		{
			e.Column.HeaderStyle.Width = Unit.Pixel(380);
			e.Column.HeaderStyle.HorizontalAlign = HorizontalAlign.Center;
		}
	...
	{
		base.Response.Redirect("BOM_Design_WO_Grid.aspx?ModId=3&SubModId=26");
	}

	protected void btnCopy_Click(object sender, EventArgs e)
	{
		base.Response.Redirect("BOM_Design_Root_Assembly_Copy_WO.aspx?WONoDest=" + woQueryStr + "&ModId=3&SubModId=26");
	}
