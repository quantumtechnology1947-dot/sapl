...
public class Module_HR_Transactions_Salary_Neha_OverTimes : Page, IRequiresSessionState
{
	...
	protected void Page_Init(object sender, EventArgs e)
	{
		...
		if (!base.IsPostBack)
		{
			try
			{
				...
				dataTable.Columns.Add(new DataColumn("OTHrs1", typeof(double)));
				dataTable.Columns.Add(new DataColumn("OTRate", typeof(double)));
				sqlConnection.Open();
				...
				for (int i = 0; i < dataSet2.Tables[0].Rows.Count; i++)
				{
					...
					if (dataSet9.Tables[0].Rows.Count > 0)
					{
						...
						if (dataSet11.Tables[0].Rows.Count > 0)
						{
							...
							num14 = Convert.ToDouble(dataSet11.Tables[0].Rows[0]["Present"]);
							num15 = Convert.ToDouble(dataSet11.Tables[0].Rows[0]["Absent"]);
							num16 = Convert.ToDouble(dataSet11.Tables[0].Rows[0]["PL"]);
							num17 = Convert.ToDouble(dataSet11.Tables[0].Rows[0]["Coff"]);
							num18 = Convert.ToDouble(dataSet11.Tables[0].Rows[0]["HalfDay"]);
							num19 = Convert.ToDouble(dataSet11.Tables[0].Rows[0]["Sunday"]);
							num20 = fun.CountSundays(year, MonthId);
							num21 = fun.GetHoliday(MonthId, CompId, FinYearId);
							num32 = Convert.ToInt32(dataSet9.Tables[0].Rows[0]["AttBonusPer1"]);
							num33 = Convert.ToInt32(dataSet9.Tables[0].Rows[0]["AttBonusPer2"]);
							num34 = Convert.ToDouble(dataSet9.Tables[0].Rows[0]["ExGratia"]);
							num44 = Convert.ToDouble(dataSet9.Tables[0].Rows[0]["VehicleAllowance"]);
							num45 = Convert.ToDouble(dataSet11.Tables[0].Rows[0]["Addition"]);
							num46 = Convert.ToDouble(dataSet11.Tables[0].Rows[0]["Deduction"]);
							num11 = num - (num15 - (num16 + num17));
							num12 = num14 + num19 + num18;
							num13 = num - num11;
							num22 = Math.Round(num2 * num11 / num);
							num23 = Math.Round(num3 * num11 / num);
							num24 = Math.Round(num4 * num11 / num);
							num25 = Math.Round(num5 * num11 / num);
							num26 = Math.Round(num6 * num11 / num);
							num27 = Math.Round(num7 * num11 / num);
							num28 = Math.Round(num22 + num23 + num24 + num25 + num26 + num27);
							num9 = fun.Pf_Cal(num28, 1, Convert.ToInt32(dataSet9.Tables[0].Rows[0]["PFEmployee"]));
							num29 = num9;
							num35 = Math.Round(num34 * num11 / num);
							num49 = Convert.ToDouble(dataSet11.Tables[0].Rows[0]["Installment"]);
							num50 = Convert.ToDouble(dataSet11.Tables[0].Rows[0]["MobileExeAmt"]);
							...
							for (int j = 0; j < dataSet12.Tables[0].Rows.Count; j++)
							{
								...
								case "2":
									...
									break;
								case "3":
									num43 += Convert.ToDouble(dataSet12.Tables[0].Rows[j]["Qty"]) * Convert.ToDouble(dataSet12.Tables[0].Rows[j]["Amount"]);
									break;
								}
							}
							if (dataSet9.Tables[0].Rows[0]["OverTime"].ToString() == "2")
							{
								...
								num10 = fun.OTRate(num8, Convert.ToDouble(dataSet13.Tables[0].Rows[0]["Hours"]), Convert.ToDouble(dataSet14.Tables[0].Rows[0]["Hours"]), num);
								num36 = Math.Round(fun.OTAmt(num10, Convert.ToDouble(dataSet11.Tables[0].Rows[0]["OverTimeHrs"])));
								dataRow[53] = Convert.ToDouble(dataSet11.Tables[0].Rows[0]["OverTimeHrs"]);
								dataRow[54] = num10;
							}
							if (num12 >= num - (num21 + num20 + 2.0) && num12 < num + 2.0 - (num21 + num20))
							{
								num30 = 1;
								num31 = Math.Round(num8 * num32 / 100.0);
							}
							else if (num12 >= num + 2.0 - (num21 + num20))
							{
								num30 = 2;
								num31 = Math.Round(num8 * num33 / 100.0);
							}
				...
				string text5 = base.Server.MapPath("~/Module/HR/Transactions/Reports/Salary_Neha_OverTime.rpt");
				report.Load(text5);
