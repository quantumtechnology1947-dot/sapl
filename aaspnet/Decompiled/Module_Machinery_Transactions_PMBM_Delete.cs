...
public class Module_Machinery_Transactions_PMBM_Delete : Page, IRequiresSessionState
{
	...
	public void fillGrid(string sql, GridView GridView2)
	{
		...
		try
		{
			...
			for (int i = 0; i < dataSet.Tables[0].Rows.Count; i++)
			{
				...
				dataRow[3] = fun.FromDateDMY(dataSet.Tables[0].Rows[i]["LastPMBMDate"].ToString());
	...
	protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		try
		{
			if (e.CommandName == "Del")
			{
				...
				string cmdText = fun.select("tblMS_PMBM_Details.Id", "tblMS_PMBM_Details,tblMS_PMBM_Master", " tblMS_PMBM_Details.MId=tblMS_PMBM_Master.Id AND tblMS_PMBM_Master.Id='" + num + "' AND CompId='" + CompId + "'");
				SqlCommand selectCommand = new SqlCommand(cmdText, con);
				SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(selectCommand);
				DataSet dataSet = new DataSet();
				sqlDataAdapter.Fill(dataSet);
				for (int i = 0; i < dataSet.Tables[0].Rows.Count; i++)
				{
					string cmdText2 = fun.delete("tblMS_PMBM_Details", " Id= '" + dataSet.Tables[0].Rows[i]["Id"].ToString() + "'");
					SqlCommand sqlCommand = new SqlCommand(cmdText2, con);
					con.Open();
					sqlCommand.ExecuteNonQuery();
					con.Close();
				}
				string cmdText3 = fun.select("*", "tblMS_PMBM_Details", " MId= '" + num + "'");
				...
				{
					string cmdText4 = fun.delete("tblMS_PMBM_Master", " Id= '" + num + "'");
