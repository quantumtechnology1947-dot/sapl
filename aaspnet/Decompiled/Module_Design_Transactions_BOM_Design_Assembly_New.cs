...
public class Module_Design_Transactions_BOM_Design_Assembly_New : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		try
		{
			...
			lblWo.Text = wono;
			string cmdText = fun.select("EquipmentNo", "tblDG_BOM_Master", "EquipmentNo is not null And EquipmentNo!='99999' Order by EquipmentNo Desc");
			SqlCommand selectCommand = new SqlCommand(cmdText, con);
			SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(selectCommand);
			DataSet dataSet = new DataSet();
			sqlDataAdapter.Fill(dataSet);
			string empty = string.Empty;
			empty = ((dataSet.Tables[0].Rows.Count <= 0 || !(dataSet.Tables[0].Rows[0]["EquipmentNo"].ToString() != "") || dataSet.Tables[0].Rows[0]["EquipmentNo"] == DBNull.Value) ? "00001" : (Convert.ToInt32(dataSet.Tables[0].Rows[0]["EquipmentNo"]) + 1).ToString("D5"));
	...
	protected void GridView2_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		try
		{
			...
			string cmdText7 = fun.select("EquipmentNo", "tblDG_BOM_Master", "EquipmentNo is not null And EquipmentNo!='99999' Order by EquipmentNo Desc");
