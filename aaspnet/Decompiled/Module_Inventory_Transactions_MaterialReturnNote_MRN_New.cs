...
public class Module_Inventory_Transactions_MaterialReturnNote_MRN_New : Page, IRequiresSessionState
{
	...
	protected void GridView2_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		try
		{
			...
			string text3 = "";
			string text4 = "";
			string cmdText = fun.select("Id,ItemId", "tblMM_Rate_Register", "ItemId='" + text + "' And CompId='" + CompId + "'");
			...
			{
				if (num != 0 && ((TextBox)gridViewRow.FindControl("txtqty")).Text != "")
				{
					double num2 = Convert.ToDouble(decimal.Parse(((TextBox)gridViewRow.FindControl("txtqty")).Text.ToString()).ToString("N3"));
					string text5 = ((TextBox)gridViewRow.FindControl("txtremarks")).Text;
					SqlCommand selectCommand2 = new SqlCommand(fun.select("Id", "tblinv_MaterialReturn_Temp", "ItemId='" + text + "' And CompId='" + CompId + "' AND SessionId='" + sId + "'"), con);
					SqlDataAdapter sqlDataAdapter2 = new SqlDataAdapter(selectCommand2);
					DataSet dataSet2 = new DataSet();
					sqlDataAdapter2.Fill(dataSet2, "tblinv_MaterialReturn_Temp");
					if (dataSet2.Tables[0].Rows.Count > 0)
					{
						string empty = string.Empty;
						empty = "Item is already selected for MRN.";
						base.ClientScript.RegisterStartupScript(GetType(), "myalert", "alert('" + empty + "');", addScriptTags: true);
					}
					else if (num == 1 && fun.NumberValidationQty(num2.ToString()))
					{
						if (((DropDownList)gridViewRow.FindControl("drpdept")).SelectedValue != "0")
						{
							...
							text4 = fun.insert("tblinv_MaterialReturn_Temp", "CompId,SessionId,ItemId,DeptId,RetQty,Remarks", "'" + CompId + "','" + sId + "','" + text + "','" + text2 + "','" + num2 + "','" + text5 + "'");
							SqlCommand sqlCommand = new SqlCommand(text4, con);
							sqlCommand.ExecuteNonQuery();
							Page.Response.Redirect(Page.Request.Url.ToString(), endResponse: true);
						}
					}
					else if (num == 2 && fun.NumberValidationQty(num2.ToString()))
					{
						text3 = ((TextBox)gridViewRow.FindControl("txtwono")).Text;
						if (fun.CheckValidWONo(text3, CompId, FinYearId))
						{
							text4 = fun.insert("tblinv_MaterialReturn_Temp", "CompId,SessionId,ItemId,WONo,RetQty,Remarks", "'" + CompId + "','" + sId + "','" + text + "','" + text3 + "','" + num2 + "','" + text5 + "'");
							SqlCommand sqlCommand2 = new SqlCommand(text4, con);
			...
			else
			{
				...
				empty4 = "Selected item is cancel due to rate is not available in ERP.";
	...
	public void loadgrid()
	{
		try
		{
			string cmdText = fun.select("Id,ItemId,DeptId,WONo,RetQty,Remarks", "tblinv_MaterialReturn_Temp", "CompId='" + CompId + "' AND SessionId='" + sId + "' order by Id desc ");
			SqlCommand selectCommand = new SqlCommand(cmdText, con);
			SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(selectCommand);
			DataSet dataSet = new DataSet();
			...
			dataTable.Columns.Add(new DataColumn("RetQty", typeof(string)));
			...
			for (int i = 0; i < dataSet.Tables[0].Rows.Count; i++)
			{
				...
				if (dataSet.Tables[0].Rows[i]["ItemId"] != DBNull.Value)
				{
					...
					if (dataSet2.Tables[0].Rows.Count > 0)
					{
						...
						text = decimal.Parse(dataSet2.Tables[0].Rows[0]["StockQty"].ToString()).ToString("N3");
				...
				{
					text2 = dataSet.Tables[0].Rows[i]["WONo"].ToString();
					dataRow[5] = text2;
				}
				else
				{
				...
				text3 = decimal.Parse(dataSet.Tables[0].Rows[i]["RetQty"].ToString()).ToString();
				dataRow[6] = text3;
				text4 = dataSet.Tables[0].Rows[i]["Remarks"].ToString();
				dataRow[7] = text4;
	...
	protected void GridView3_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		try
		{
			if (e.CommandName == "Del")
			{
				...
				string cmdText = fun.delete("tblinv_MaterialReturn_Temp", "Id='" + num + "' AND SessionId='" + sId + "' AND CompId='" + CompId + "'");
				SqlCommand sqlCommand = new SqlCommand(cmdText, con);
				sqlCommand.ExecuteNonQuery();
				Page.Response.Redirect(Page.Request.Url.ToString(), endResponse: true);
			}
			string cmdText2 = fun.select("MRNNo", "tblInv_MaterialReturn_Master", "CompId='" + CompId + "' AND FinYearId='" + FinYearId + "'  Order by MRNNo Desc");
			SqlCommand selectCommand = new SqlCommand(cmdText2, con);
			SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(selectCommand);
			DataSet dataSet = new DataSet();
			sqlDataAdapter.Fill(dataSet, "tblInv_MaterialReturn_Master");
			string text = "";
			...
			int num2 = 1;
			string text2 = "";
			string cmdText3 = fun.select("*", "tblinv_MaterialReturn_Temp", "SessionId='" + sId + "' AND CompId='" + CompId + "'");
			...
			if (dataSet2.Tables[0].Rows.Count > 0)
			{
				for (int i = 0; i < dataSet2.Tables[0].Rows.Count; i++)
				{
					if (num2 == 1)
					{
						string cmdText4 = "Insert into tblInv_MaterialReturn_Master(SysDate,SysTime,CompId,FinYearId,SessionId,MRNNo) VALUES  ('" + CDate + "','" + CTime + "','" + CompId + "','" + FinYearId + "','" + sId + "','" + text + "')";
						SqlCommand sqlCommand2 = new SqlCommand(cmdText4, con);
						sqlCommand2.ExecuteNonQuery();
						num2 = 0;
						string cmdText5 = fun.select("Id", "tblInv_MaterialReturn_Master", "CompId='" + CompId + "' Order by Id desc");
						SqlCommand selectCommand3 = new SqlCommand(cmdText5, con);
						SqlDataAdapter sqlDataAdapter3 = new SqlDataAdapter(selectCommand3);
						DataSet dataSet3 = new DataSet();
						sqlDataAdapter3.Fill(dataSet3, "tblInv_MaterialReturn_Master");
						text2 = dataSet3.Tables[0].Rows[0]["Id"].ToString();
					}
					string cmdText6 = "Insert into tblInv_MaterialReturn_Details(MId,MRNNo,ItemId,DeptId,WONo,RetQty,Remarks) VALUES  ('" + text2 + "','" + text + "','" + dataSet2.Tables[0].Rows[i]["ItemId"].ToString() + "','" + dataSet2.Tables[0].Rows[i]["DeptId"].ToString() + "','" + dataSet2.Tables[0].Rows[i]["WONo"].ToString() + "','" + Convert.ToDouble(decimal.Parse(dataSet2.Tables[0].Rows[i]["RetQty"].ToString()).ToString("N3")) + "','" + dataSet2.Tables[0].Rows[i]["Remarks"].ToString() + "')";
					SqlCommand sqlCommand3 = new SqlCommand(cmdText6, con);
					sqlCommand3.ExecuteNonQuery();
				}
			}
			string cmdText7 = fun.delete("tblinv_MaterialReturn_Temp", "CompId='" + CompId + "'And SessionId='" + sId + "'");
