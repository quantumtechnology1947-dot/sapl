...
public class Module_Design_Transactions_TPL_Amd : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		try
		{
			...
			TPLId = base.Request.QueryString["Id"].ToString();
			lblWONo.Text = WONo;
			if (!base.IsPostBack)
			{
				BindDataCust();
	...
	public void BindDataCust()
	{
		try
		{
			...
			sqlConnection.Open();
			string cmdText = fun.select("*", "tblDG_TPL_Amd", "FinYearId<='" + FinYearId + "' And CompId='" + CompId + "' And TPLId='" + TPLId + "' And WONo='" + WONo + "' And ItemId='" + ItemId + "'");
