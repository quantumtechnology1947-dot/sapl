...
public class Module_ProjectManagement_Transactions_MaterialCreditNote_MCN_Delete_Details : Page, IRequiresSessionState
{
	...
	public void loaddata()
	{
		try
		{
			...
			for (int i = 0; i < dataSet.Tables[0].Rows.Count; i++)
			{
				...
				dataRow[14] = dataSet.Tables[0].Rows[i]["MId"].ToString();
	...
	{
		base.Response.Redirect("~/Module/ProjectManagement/Transactions/MaterialCreditNote_MCN_Delete.aspx?ModId=7&SubModId=127");
	}
	...
	protected void GridView1_RowDeleting(object sender, GridViewDeleteEventArgs e)
	{
		try
		{
			...
			SqlCommand sqlCommand = new SqlCommand(fun.delete("tblPM_MaterialCreditNote_Details", "Id='" + num + "'"), con);
			con.Open();
			sqlCommand.ExecuteNonQuery();
			con.Close();
			string cmdText = fun.select("*", "tblPM_MaterialCreditNote_Details", "MId='" + num2 + "'");
			SqlCommand selectCommand = new SqlCommand(cmdText, con);
			SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(selectCommand);
			DataSet dataSet = new DataSet();
			sqlDataAdapter.Fill(dataSet);
			...
			SqlCommand sqlCommand2 = new SqlCommand(fun.delete("tblPM_MaterialCreditNote_Master", "Id='" + num2 + "'"), con);
			con.Open();
