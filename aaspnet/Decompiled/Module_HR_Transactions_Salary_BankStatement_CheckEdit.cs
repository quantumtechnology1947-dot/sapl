...
public class Module_HR_Transactions_Salary_BankStatement_CheckEdit : Page, IRequiresSessionState
{
	...
	public void loaddata()
	{
		try
		{
			...
			for (int i = 0; i < dataSet.Tables[0].Rows.Count; i++)
			{
				...
				dataRow[1] = dataSet.Tables[0].Rows[i]["Title"].ToString() + ". " + dataSet.Tables[0].Rows[i]["EmployeeName"].ToString();
				double num2 = 0.0;
				double num3 = 0.0;
				double num4 = 0.0;
				double num5 = 0.0;
				...
				if (dataSet4.Tables[0].Rows.Count > 0)
				{
					...
					if (dataSet5.Tables[0].Rows.Count > 0)
					{
						...
						dataRow[2] = Math.Round(num43 - num44);
					}
				}
				dataRow[3] = dataSet.Tables[0].Rows[i]["BankAccountNo"].ToString();
		...
		try
		{
			...
			foreach (GridViewRow row in GridView2.Rows)
			{
				if (!((CheckBox)row.FindControl("CheckBox1")).Checked)
				{
					...
					string cmdText = fun.update("tblHR_Salary_Master", "ReleaseFlag=0,ChequeNo=Null,ChequeNoDate=Null,BankId=Null,EmpDirect=Null,TransNo=0", "Id='" + text + "'");
