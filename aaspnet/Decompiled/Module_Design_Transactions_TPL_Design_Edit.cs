...
public class Module_Design_Transactions_TPL_Design_Edit : Page, IRequiresSessionState
{
	...
	public void BindDataCust(string Cid, string EID)
	{
		try
		{
			...
			for (int i = 0; i < dataSet.Tables[0].Rows.Count; i++)
			{
				...
				{
					string cmdText2 = fun.select("CustomerName,CustomerId", "SD_Cust_Master", "CompId='" + num + "'  AND  CustomerId='" + dataSet.Tables[0].Rows[i]["CustomerId"].ToString() + "'");
					...
					string cmdText4 = fun.select("Title+'.'+EmployeeName As EmpLoyeeName", "tblHR_OfficeStaff", string.Concat("CompId='", num, "' AND EmpId='", dataSet.Tables[0].Rows[i]["SessionId"], "'"));
