...
public class Module_Accounts_Transactions_TourVoucher_Edit_Details : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		try
		{
			...
			string cmdText = fun.select("*", "tblACC_TourIntimation_Master", "Id='" + TIMid + "'  And   CompId='" + CompId + "' ");
			...
			string cmdText6 = fun.select("AmtBalTowardsCompany,AmtBalTowardsEmployee", "tblACC_TourVoucher_Master", " Id='" + id + "'");
			...
			{
				txtAmtBalTowardsCompany.Text = dataSet7.Tables[0].Rows[0]["AmtBalTowardsCompany"].ToString();
				txtAmtBalTowardsEmployee.Text = dataSet7.Tables[0].Rows[0]["AmtBalTowardsEmployee"].ToString();
			}
			FillGridAdvanceTo();
			fillgrid();
	...
	public void fillgrid()
	{
		...
		try
		{
			...
			dataTable.Columns.Add(new DataColumn("TVADId", typeof(int)));
			...
			dataTable.Columns.Add(new DataColumn("SanctionedAmount", typeof(double)));
			dataTable.Columns.Add(new DataColumn("SanctionedRemarks", typeof(string)));
			dataTable.Columns.Add(new DataColumn("TDMId", typeof(int)));
			double num = 0.0;
			double num2 = 0.0;
			for (int i = 0; i < dataSet.Tables[0].Rows.Count; i++)
			{
				...
				string cmdText2 = fun.select("*", "tblACC_TourAdvance_Details", "ExpencessId='" + Convert.ToInt32(dataSet.Tables[0].Rows[i]["Id"]) + "' AND MId='" + TIMid + "'");
				...
				string cmdText3 = fun.select("*", "tblACC_TourVoucherAdvance_Details", "MId='" + id + "' AND TDMId='" + Convert.ToInt32(dataSet.Tables[0].Rows[i]["Id"]) + "'");
				...
				if (dataSet3.Tables[0].Rows.Count > 0)
				{
					dataRow[0] = Convert.ToInt32(dataSet3.Tables[0].Rows[0]["Id"]);
					dataRow[4] = Convert.ToDouble(dataSet3.Tables[0].Rows[0]["SanctionedAmount"]);
					dataRow[5] = dataSet3.Tables[0].Rows[0]["Remarks"].ToString();
					dataRow[6] = Convert.ToInt32(dataSet3.Tables[0].Rows[0]["TDMId"]);
					num2 += Convert.ToDouble(dataSet3.Tables[0].Rows[0]["SanctionedAmount"]);
				}
	...
	public void FillGridAdvanceTo()
	{
		try
		{
			DataTable dataTable = new DataTable();
			con.Open();
			string cmdText = fun.select("*", "tblACC_TourVoucherAdvance", "MId='" + id + "' ");
			SqlCommand selectCommand = new SqlCommand(cmdText, con);
			SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(selectCommand);
			DataSet dataSet = new DataSet();
			sqlDataAdapter.Fill(dataSet);
			dataTable.Columns.Add(new DataColumn("TVTId", typeof(int)));
			...
			for (int i = 0; i < dataSet.Tables[0].Rows.Count; i++)
			{
				...
				string cmdText2 = fun.select("*", "tblACC_TourAdvance", "Id='" + Convert.ToInt32(dataSet.Tables[0].Rows[i]["TAMId"]) + "'");
				SqlCommand selectCommand2 = new SqlCommand(cmdText2, con);
				SqlDataAdapter sqlDataAdapter2 = new SqlDataAdapter(selectCommand2);
				DataSet dataSet2 = new DataSet();
				sqlDataAdapter2.Fill(dataSet2);
				if (dataSet2.Tables[0].Rows.Count > 0)
				{
					...
					string cmdText3 = fun.select("Title+'. '+EmployeeName+ ' ['+ EmpId +'] ' As EmpLoyeeName", "tblHR_OfficeStaff", string.Concat("CompId='", CompId, "'AND FinYearId<='", FinYearId, "' AND EmpId='", dataSet2.Tables[0].Rows[0]["EmpId"], "'"));
					SqlCommand selectCommand3 = new SqlCommand(cmdText3, con);
					SqlDataAdapter sqlDataAdapter3 = new SqlDataAdapter(selectCommand3);
					DataSet dataSet3 = new DataSet();
					sqlDataAdapter3.Fill(dataSet3);
					if (dataSet3.Tables[0].Rows.Count > 0)
					{
						dataRow[1] = dataSet3.Tables[0].Rows[0]["EmployeeName"].ToString();
					}
					dataRow[2] = Convert.ToDouble(decimal.Parse(dataSet2.Tables[0].Rows[0]["Amount"].ToString()).ToString("N3"));
					dataRow[3] = dataSet2.Tables[0].Rows[0]["Remarks"].ToString();
					dataRow[4] = Convert.ToDouble(dataSet.Tables[0].Rows[i]["SanctionedAmount"]);
					dataRow[5] = dataSet.Tables[0].Rows[i]["Remarks"].ToString();
					dataRow[6] = dataSet.Tables[0].Rows[i]["TAMId"].ToString();
					num += Convert.ToDouble(dataSet2.Tables[0].Rows[0]["Amount"]);
					num2 += Convert.ToDouble(dataSet.Tables[0].Rows[i]["SanctionedAmount"]);
					dataTable.Rows.Add(dataRow);
					dataTable.AcceptChanges();
				}
			}
	...
	protected void btnSubmit_Click(object sender, EventArgs e)
	{
		try
		{
			...
			if (txtAmtBalTowardsCompany.Text != "" && fun.NumberValidationQty(txtAmtBalTowardsCompany.Text) && txtAmtBalTowardsEmployee.Text != "" && fun.NumberValidationQty(txtAmtBalTowardsEmployee.Text) && num2 - num == 0 && num4 - num3 == 0)
			{
				...
				string cmdText = fun.update("tblACC_TourVoucher_Master", " SysDate='" + CDate + "',SysTime='" + CTime + "', SessionId='" + sId + "', AmtBalTowardsCompany='" + num5 + "', AmtBalTowardsEmployee='" + num6 + "'", "CompId='" + CompId + "' And Id='" + id + "'");
				SqlCommand sqlCommand = new SqlCommand(cmdText, con);
				sqlCommand.ExecuteNonQuery();
				con.Close();
				foreach (GridViewRow row5 in GridView2.Rows)
				{
					int num13 = Convert.ToInt32(((Label)row5.FindControl("lblTDMId")).Text);
					string text = ((TextBox)row5.FindControl("txtRemarks")).Text;
					...
					{
						double num14 = Convert.ToDouble(decimal.Parse(((TextBox)row5.FindControl("txtAmount")).Text).ToString("N2"));
						string cmdText2 = fun.update("tblACC_TourVoucherAdvance_Details", "SanctionedAmount='" + num14 + "',Remarks='" + text + "'", "MId='" + id + "' AND TDMId='" + num13 + "'");
				...
				{
					int num15 = Convert.ToInt32(((Label)row6.FindControl("lblTATId")).Text);
					string text2 = ((TextBox)row6.FindControl("txtATRemarks")).Text;
					...
					{
						double num16 = Convert.ToDouble(decimal.Parse(((TextBox)row6.FindControl("txtATAmount")).Text).ToString("N2"));
						string cmdText3 = fun.update("tblACC_TourVoucherAdvance", "SanctionedAmount='" + num16 + "',Remarks='" + text2 + "'", "MId='" + id + "' AND TAMId='" + num15 + "' ");
				...
				base.Response.Redirect("TourVoucher_Edit.aspx?ModId=11&SubModId=126");
			}
			else
			{
