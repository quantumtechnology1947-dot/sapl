...
public class Module_Design_Transactions_BOM_Design_CopyWo : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		try
		{
			...
			if (!Page.IsPostBack)
			{
				...
				{
					SearchGridView1.Rows[i].Cells[6].Text = "<a href='BOM_Design_Copy_Tree.aspx?WONoSrc=" + SearchGridView1.Rows[i].Cells[6].Text + "&WONoDest=" + WoNoDest + "&DestPId=" + pid + "&DestCId=" + childid + "&ModId=3&SubModId=26'>" + SearchGridView1.Rows[i].Cells[6].Text + "</a>";
				}
			}
		}
		catch (Exception)
	...
	{
		Page.ClientScript.RegisterClientScriptBlock(GetType(), "myUniqueKey", "self.parent.location='BOM_Design_WO_TreeView.aspx?WONo=" + WoNoDest + "&ModId=3&SubModId=26';", addScriptTags: true);
	}
	...
	public void BindDataCust(string Cid, string EID)
	{
		try
		{
			...
			for (int i = 0; i < dataSet.Tables[0].Rows.Count; i++)
			{
				...
				{
					string cmdText2 = fun.select("CustomerName,CustomerId", "SD_Cust_Master", "CompId='" + CompId + "'AND FinYearId<='" + FinYearId + "'AND  CustomerId='" + dataSet.Tables[0].Rows[i]["CustomerId"].ToString() + "'");
					...
					string cmdText4 = fun.select("Title+'.'+EmployeeName As EmpLoyeeName", "tblHR_OfficeStaff", string.Concat("CompId='", CompId, "'AND FinYearId<='", FinYearId, "' AND EmpId='", dataSet.Tables[0].Rows[i]["SessionId"], "'"));
