...
public class Module_SalesDistribution_Masters_CustomerMaster_Print : Page, IRequiresSessionState
{
	...
	public void BindDataCust(string Cid)
	{
		try
		{
			...
			string cmdText = fun.select("RegdAddress,RegdCountry,RegdState,RegdCity,CustomerId,CustomerName,SessionId,FinYearId,REPLACE(CONVERT(varchar, CONVERT(datetime, SUBSTRING(SD_Cust_master.SysDate, CHARINDEX('-', SD_Cust_master.SysDate) + 1, 2) + '-' + LEFT(SD_Cust_master.SysDate,CHARINDEX('-', SD_Cust_master.SysDate) - 1) + '-' + RIGHT(SD_Cust_master.SysDate, CHARINDEX('-', REVERSE(SD_Cust_master.SysDate)) - 1)), 103), '/', '-')AS SysDate", "SD_Cust_master", "FinYearId<=" + FinYearId + " AND CompId='" + CompId + "'" + text + " Order By SalesId Desc");
			...
			for (int i = 0; i < dataSet.Tables[0].Rows.Count; i++)
			{
				...
				if (dataSet.Tables[0].Rows.Count > 0)
				{
					...
					dataRow[5] = value;
					dataRow[6] = fun.GetRandomAlphaNumeric();
	...
	protected void btnPrintAll_Click(object sender, EventArgs e)
	{
		...
		base.Response.Redirect("~/Module/SalesDistribution/Masters/Customer_Details_Print_All.aspx?ModId=2&SubModId=7&Key=" + randomAlphaNumeric);
	}
}
