...
public class Module_Accounts_Transactions_ServiceTaxInvoice_New : Page, IRequiresSessionState
{
			...
			if (dataSet.Tables[0].Rows.Count > 0)
			{
				for (int i = 0; i < dataSet.Tables[0].Rows.Count; i++)
				{
					...
					for (int j = 0; j < dataSet4.Tables[0].Rows.Count; j++)
					{
						...
						{
							num = Convert.ToDouble(dataSet5.Tables[0].Rows[0]["ReqQty"]);
						}
	...
	protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		try
		{
			if (e.CommandName == "Sel")
			{
				...
				if (text4 != "")
				{
					base.Response.Redirect("ServiceTaxInvoice_New_Details.aspx?poid=" + base.Server.UrlEncode(fun.Encrypt(text2)) + "&wn=" + base.Server.UrlEncode(fun.Encrypt(text4)) + "&pn=" + base.Server.UrlEncode(fun.Encrypt(text)) + "&date=" + base.Server.UrlEncode(fun.Encrypt(text3)) + "&cid=" + base.Server.UrlEncode(fun.Encrypt(text5)) + "&ModId=11&SubModId=52");
				}
				else
				{
