...
public class Module_Design_Transactions_TPL_Design_Assembly_Edit : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		try
		{
			...
			lblWONo.Text = wono;
			string sql = fun.select("tblDG_TPL_Master.Id,tblDG_Item_Master.UOMBasic,tblDG_TPL_Master.AmdNo,tblDG_TPL_Master.EquipmentNo,tblDG_TPL_Master.UnitNo,tblDG_TPL_Master.PartNo,tblDG_TPL_Master.CId,tblDG_TPL_Master.ItemId,tblDG_Item_Master.AttName,tblDG_Item_Master.FileName,tblDG_Item_Master.ManfDesc,Unit_Master.Symbol As UOM,tblDG_TPL_Master.Qty", "tblDG_Item_Master,tblDG_TPL_Master,Unit_Master", "tblDG_Item_Master.Id=tblDG_TPL_Master.ItemId AND WONo='" + wono + "' AND PId='0' AND tblDG_TPL_Master.CompId='" + CompId + "'AND tblDG_TPL_Master.FinYearId<='" + FinYearId + "' And tblDG_Item_Master.Id='" + ItemId + "' AND tblDG_Item_Master.UOMBasic = Unit_Master.Id Order by tblDG_TPL_Master.ItemId Desc");
			if (!base.IsPostBack)
			{
			...
			foreach (GridViewRow row in GridView2.Rows)
			{
				...
				string cmdText = fun.select("tblDG_Item_Master.FileName,tblDG_Item_Master.AttName ", "tblDG_Item_Master,tblDG_TPL_Master", "tblDG_Item_Master.CompId='" + CompId + "' And tblDG_Item_Master.Id=tblDG_TPL_Master.ItemId     And tblDG_TPL_Master.WONo='" + wono + "' and tblDG_Item_Master.FinYearId<='" + FinYearId + "'   And  tblDG_TPL_Master.ItemId='" + num + "' ");
	...
	protected void GridView2_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		try
		{
			...
			if (e.CommandName == "Upd")
			{
				...
				sqlConnection.Open();
				if (fun.NumberValidationQty(((TextBox)gridViewRow2.FindControl("txtQuntity")).Text) && ((TextBox)gridViewRow2.FindControl("txtQuntity")).Text != "" && ((TextBox)gridViewRow2.FindControl("txtManfDescription")).Text != "")
				{
					double num = Convert.ToDouble(decimal.Parse(((TextBox)gridViewRow2.FindControl("txtQuntity")).Text.ToString()).ToString("N3"));
					string text = ((TextBox)gridViewRow2.FindControl("txtManfDescription")).Text;
					...
					if (((FileUpload)gridViewRow2.FindControl("DrwUpload")).PostedFile != null)
					{
						...
						string cmdText = fun.update("tblDG_Item_Master", "FileName='" + text2 + "',FileSize='" + array.Length + "',ContentType='" + postedFile.ContentType + "',FileData= @Data  ", "Id='" + num3 + "'");
					...
					if (((FileUpload)gridViewRow2.FindControl("OtherUpload")).PostedFile != null)
					{
						...
						num4 = array2.Length;
						text4 = postedFile2.ContentType;
						string cmdText2 = fun.update("tblDG_Item_Master", "AttName='" + text3 + "',AttSize='" + num4 + "',AttContentType='" + text4 + "',AttData= @AttData  ", "Id='" + num3 + "'");
						SqlCommand sqlCommand2 = new SqlCommand(cmdText2, sqlConnection);
						sqlCommand2.Parameters.AddWithValue("@AttData", array2);
						sqlCommand2.ExecuteNonQuery();
					}
					string text5 = fun.FromDate(fun.getOpeningDate(CompId, FinYearId));
					string cmdText3 = fun.select("*", "tblDG_TPL_Master", "Id='" + num2 + "'");
					SqlCommand selectCommand = new SqlCommand(cmdText3, sqlConnection);
					SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(selectCommand);
					DataSet dataSet = new DataSet();
					sqlDataAdapter.Fill(dataSet);
					string text6 = ((Label)gridViewRow2.FindControl("Label3")).Text;
					string text7 = ((Label)gridViewRow2.FindControl("LblUom")).Text;
					string text8 = "";
					text8 = ((dataSet.Tables[0].Rows.Count <= 0) ? "0" : (Convert.ToInt32(dataSet.Tables[0].Rows[0]["AmdNo"].ToString()) + 1).ToString());
					string cmdText4 = fun.insert("tblDG_TPL_Amd", "SysDate,SysTime,SessionId,CompId,FinYearId,WONo,TPLId,PId,CId,ItemId,Description,UOM,AmdNo,Qty", "'" + currDate.ToString() + "','" + currTime.ToString() + "','" + sId.ToString() + "','" + CompId + "','" + FinYearId + "','" + wono + "','" + num2 + "','" + dataSet.Tables[0].Rows[0]["PId"].ToString() + "','" + dataSet.Tables[0].Rows[0]["CId"].ToString() + "', '" + dataSet.Tables[0].Rows[0]["ItemId"].ToString() + "' , '" + text6 + "', '" + text7 + "','" + dataSet.Tables[0].Rows[0]["AmdNo"].ToString() + "','" + dataSet.Tables[0].Rows[0]["Qty"].ToString() + "' ");
					SqlCommand sqlCommand3 = new SqlCommand(cmdText4, sqlConnection);
					sqlCommand3.ExecuteNonQuery();
					string cmdText5 = fun.update("tblDG_Item_Master", "SysDate='" + currDate.ToString() + "',SysTime='" + currTime.ToString() + "',SessionId='" + sId.ToString() + "',CompId='" + CompId + "',FinYearId='" + FinYearId + "',ManfDesc='" + text.ToString() + "',UOMBasic='" + selectedValue + "',OpeningBalDate='" + text5 + "',OpeningBalQty='0'", "Id='" + num3 + "'");
					SqlCommand sqlCommand4 = new SqlCommand(cmdText5, sqlConnection);
					sqlCommand4.ExecuteNonQuery();
					string cmdText6 = fun.update("tblDG_TPL_Master", "SessionId='" + sId.ToString() + "',CompId='" + CompId + "',FinYearId='" + FinYearId + "',WONo='" + wono + "',Qty='" + num + "',AmdNo='" + text8 + "'", "Id='" + num2 + "'");
			...
			if (e.CommandName == "Del")
			{
				...
				string cmdText8 = fun.delete("tblDG_TPL_Master", "Id='" + num6 + "'");
	...
	{
		Page.Response.Redirect("TPL_Design_WO_TreeView_Edit.aspx?WONo=" + wono + "&ModId=3&SubModId=23");
	}
}
