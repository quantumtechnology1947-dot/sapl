...
public class Module_HR_Transactions_BankLoan_Delete : Page, IRequiresSessionState
{
		...
		try
		{
			...
			if (num > 0)
			{
				foreach (GridViewRow row2 in GridView2.Rows)
				{
					...
					{
						string text = ((Label)row2.FindControl("lblId")).Text;
						string cmdText = fun.delete("tblHR_BankLoan", "Id='" + text + "'");
						SqlCommand sqlCommand = new SqlCommand(cmdText, sqlConnection);
						sqlCommand.ExecuteNonQuery();
					}
				}
				base.Response.Redirect("BankLoan_Delete.aspx?ModId=12&SubModId=129");
			}
			else
			{
