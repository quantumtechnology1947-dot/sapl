...
public class Module_MaterialManagement_Transactions_PO_Check : Page, IRequiresSessionState
{
	...
	private string parentPage = "PO_Check.aspx";
	...
	public void makegrid(string sprno, string empid)
	{
		try
		{
			...
			string cmdText = fun.select("*", "tblMM_PO_Master", "FinYearId<='" + FyId + "' AND CompId='" + CompId + "'" + text + " AND Checked='0' Order by Id Desc");
	...
	protected void GridView2_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		try
		{
			...
			if (e.CommandName == "check")
			{
				foreach (GridViewRow row in GridView2.Rows)
				{
					...
					if (checkBox.Checked)
					{
						...
						string cmdText = fun.update("tblMM_PO_Master", "Checked='1',CheckedBy='" + sId + "',CheckedDate='" + currDate + "',CheckedTime='" + currTime + "'", "CompId='" + CompId + "' AND PONo='" + text + "' AND Id='" + text2 + "' ");
			...
			if (e.CommandName == "view")
			{
				...
				string cmdText2 = fun.select("PRSPRFlag", "tblMM_PO_Master", "Id='" + text5 + "' AND SupplierId='" + text4 + "' And PONo='" + text3 + "' AND CompId='" + CompId + "'");
