...
public class Module_MaterialManagement_Transactions_SPR_Print : Page, IRequiresSessionState
{
	...
	public void LoadData(string SprNo, string empid)
	{
		try
		{
			...
			string cmdText = fun.select("Id,SessionId,FinYearId,SPRNo,REPLACE(CONVERT(varchar, CONVERT(datetime, SUBSTRING(tblMM_SPR_Master.SysDate, CHARINDEX('-', tblMM_SPR_Master.SysDate) + 1, 2) + '-' + LEFT(tblMM_SPR_Master.SysDate,CHARINDEX('-', tblMM_SPR_Master.SysDate) - 1) + '-' + RIGHT(tblMM_SPR_Master.SysDate, CHARINDEX('-', REVERSE(tblMM_SPR_Master.SysDate)) - 1)), 103), '/', '-') AS SysDate,SysTime AS Time", "tblMM_SPR_Master", " FinYearId<='" + FyId + "' And CompId='" + CompId + "'" + text + text2 + "Order by Id Desc");
	...
	protected void GridView2_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		if (e.CommandName == "Sel")
		{
			...
			base.Response.Redirect("~/Module/MaterialManagement/Transactions/SPR_Print_Details.aspx?SPRNo=" + text + "&ModId=6&SubModId=31&Id=" + text2 + "&Key=" + randomAlphaNumeric + "&f=2");
		}
	}
}
