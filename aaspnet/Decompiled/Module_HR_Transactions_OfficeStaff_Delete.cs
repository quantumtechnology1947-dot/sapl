...
public class Module_HR_Transactions_OfficeStaff_Delete : Page, IRequiresSessionState
{
	...
	protected void GridView2_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		try
		{
			if (e.CommandName == "Deletes")
			{
				...
				string text = ((Label)gridViewRow.FindControl("lblUserId")).Text;
				SqlCommand selectCommand = new SqlCommand(fun.select("UserId", "Message", "UserId = '" + text + "'"), sqlConnection);
				SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(selectCommand);
				DataSet dataSet = new DataSet();
				sqlDataAdapter.Fill(dataSet);
				if (dataSet.Tables[0].Rows.Count > 0)
				{
					SqlCommand sqlCommand = new SqlCommand(fun.delete("Message", "UserId = '" + text + "'"), sqlConnection);
					sqlCommand.ExecuteNonQuery();
				}
				SqlCommand sqlCommand2 = new SqlCommand(fun.delete("tblHR_OfficeStaff", "UserId = '" + text + "'"), sqlConnection);
