...
public class Module_SalesDistribution_Transactions_Quotation_Print : Page, IRequiresSessionState
{
	...
	protected void SearchGridView1_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		if (e.CommandName == "sel")
		{
			...
			string text3 = ((LinkButton)gridViewRow.FindControl("lnkButton")).Text;
			string text4 = ((Label)gridViewRow.FindControl("lblId")).Text;
			string randomAlphaNumeric = fun.GetRandomAlphaNumeric();
			base.Response.Redirect("~/Module/SalesDistribution/Transactions/Quotation_Print_Details.aspx?CustomerId=" + text + "&EnqId=" + text2 + "&QuotationNo=" + text3 + "&Id=" + text4 + "&Key=" + randomAlphaNumeric + "&ModId=2&SubModId=63&parentpage=2");
		}
	}
}
