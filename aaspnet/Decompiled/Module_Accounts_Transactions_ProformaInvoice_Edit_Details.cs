...
public class Module_Accounts_Transactions_ProformaInvoice_Edit_Details : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		...
		try
		{
			...
			string cmdText2 = fun.select("Against", "tblACC_InvoiceAgainst", "Id='" + dataSet.Tables[0].Rows[0]["InvoiceMode"].ToString() + "'");
		...
		try
		{
			...
			foreach (GridViewRow row in GridView1.Rows)
			{
				if (((CheckBox)row.FindControl("CheckBox1")).Checked && ((TextBox)row.FindControl("TxtAmt")).Text != "" && ((TextBox)row.FindControl("TxtReqQty")).Text != "")
				{
					...
					if (dataSet.Tables[0].Rows.Count > 0)
					{
						double num10 = Convert.ToDouble(decimal.Parse(dataSet.Tables[0].Rows[0]["Amt"].ToString()).ToString("N2"));
						num9 = 100.0 - num10;
					}
					num5 = Convert.ToDouble(decimal.Parse(((TextBox)row.FindControl("TxtAmt")).Text.ToString()).ToString("N2"));
					string cmdText3 = fun.select("Sum(tblACC_ProformaInvoice_Details.ReqQty) as ReqQty", "tblACC_ProformaInvoice_Master,tblACC_ProformaInvoice_Details", "tblACC_ProformaInvoice_Details.MId=tblACC_ProformaInvoice_Master.Id  And  tblACC_ProformaInvoice_Master.CompId='" + CompId + "'  And tblACC_ProformaInvoice_Details.ItemId='" + num8 + "' Group By tblACC_ProformaInvoice_Details.ItemId");
					...
					if (dataSet2.Tables[0].Rows.Count > 0)
					{
						num11 = Convert.ToDouble(decimal.Parse(dataSet2.Tables[0].Rows[0]["ReqQty"].ToString()).ToString("N3"));
					}
					num3 = num4 - num11;
					if (num3 + num2 >= num7 && num5 <= num6 + num9)
					{
						int num12 = Convert.ToInt32(((Label)row.FindControl("lblId")).Text);
						...
						string cmdText4 = fun.update("tblACC_ProformaInvoice_Details", "ReqQty='" + num7 + "',Unit='" + selectedValue + "',AmtInPer='" + num5 + "'", "Id='" + num12 + "' And MId='" + Invid + "'");
			...
			{
				base.Response.Redirect("ProformaInvoice_Edit.aspx?ModId=11&SubModId=104");
			}
		}
		catch (Exception)
		{
