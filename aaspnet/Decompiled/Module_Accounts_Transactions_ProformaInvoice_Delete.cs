...
public class Module_Accounts_Transactions_ProformaInvoice_Delete : Page, IRequiresSessionState
{
	...
	public void bindgrid(string Cid, string wn)
	{
		...
		try
		{
			...
			string cmdText = fun.select("Id,FinYearId,SysDate,InvoiceNo,WONo,PONo,CustomerCode", "tblACC_ProformaInvoice_Master", "CompId='" + CompId + "' And FinYearId<='" + FinYearId + "'" + text + text2 + text3 + " Order By Id Desc");
	...
	protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		if (e.CommandName == "Sel")
		{
			try
			{
				...
				base.Response.Redirect("ProformaInvoice_Delete_Details.aspx?InvNo=" + base.Server.UrlEncode(fun.Encrypt(text)) + "&InvId=" + base.Server.UrlEncode(fun.Encrypt(text2)) + "&cid=" + base.Server.UrlEncode(fun.Encrypt(text3)) + "&ModId=11&SubModId=104");
			}
			catch (Exception)
			{
			}
