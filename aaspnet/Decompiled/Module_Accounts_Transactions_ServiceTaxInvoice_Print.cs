...
public class Module_Accounts_Transactions_ServiceTaxInvoice_Print : Page, IRequiresSessionState
{
	...
	public void bindgrid(string Cid, string wn)
	{
		...
		try
		{
			...
			string cmdText = fun.select("Id,FinYearId,SysDate,InvoiceNo,WONo,PONo,CustomerCode", "tblACC_ServiceTaxInvoice_Master", "CompId='" + CompId + "' And FinYearId<='" + FinYearId + "'" + text + text2 + text3 + " Order By Id Desc");
			...
			if (dataSet.Tables[0].Rows.Count > 0)
			{
				for (int i = 0; i < dataSet.Tables[0].Rows.Count; i++)
				{
					...
					string text4 = dataSet.Tables[0].Rows[i]["WONo"].ToString();
					...
					for (int j = 0; j < array.Length - 1; j++)
					{
						...
						{
							text5 = text5 + dataSet4.Tables[0].Rows[0][0].ToString() + ",";
						}
					}
					dataRow[5] = text5;
	...
	protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		if (e.CommandName == "Sel")
		{
			try
			{
				...
				string selectedValue = ((DropDownList)gridViewRow.FindControl("DrpPrintType")).SelectedValue;
				base.Response.Redirect("ServiceTaxInvoice_Print_Details.aspx?InvNo=" + base.Server.UrlEncode(fun.Encrypt(text)) + "&invid=" + base.Server.UrlEncode(fun.Encrypt(text2)) + "&cid=" + base.Server.UrlEncode(fun.Encrypt(text3)) + "&PT=" + base.Server.UrlEncode(fun.Encrypt(selectedValue)) + "&ModId=11&SubModId=52&Key=" + randomAlphaNumeric);
			}
			catch (Exception)
			{
			}
