...
public class Module_MaterialManagement_Transactions_PO_Edit_Supplier : Page, IRequiresSessionState
{
	...
	public void LoadData(string supcode)
	{
		...
		try
		{
			...
			{
				double num = Convert.ToDouble(((Label)row.FindControl("lblpoitems")).Text);
				if (num == 0.0)
				{
					((LinkButton)row.FindControl("LinkSup")).Visible = false;
				}
				else
				{
					((LinkButton)row.FindControl("LinkSup")).Visible = true;
				}
			}
			sqlConnection.Close();
		}
	...
	protected void GridView5_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		try
		{
			if (e.CommandName == "sel")
			{
				...
				base.Response.Redirect("PO_Edit.aspx?Code=" + text + "&ModId=6&SubModId=35");
			}
		}
		catch (Exception)
		{
