...
public class Module_HR_TourIntimation_Delete : Page, IRequiresSessionState
{
	...
	public void binddata(string Search, string EmpId)
	{
		...
		try
		{
			...
			string cmdText = fun.select("*", "tblACC_TourIntimation_Master", " SessionId='" + SId + "' And   CompId='" + CompId + "' And FinYearId<='" + FyId + "'" + text2 + text + " Order by Id Desc ");
			...
			for (int i = 0; i < dataSet.Tables[0].Rows.Count; i++)
			{
				...
				string selectCommandText = fun.select("Title+'. '+EmployeeName+ ' ['+ EmpId +'] ' As EmpLoyeeName", "tblHR_OfficeStaff", string.Concat("CompId='", CompId, "'AND FinYearId<='", FyId, "' AND EmpId='", dataSet.Tables[0].Rows[i]["EmpId"], "'"));
				SqlDataAdapter sqlDataAdapter3 = new SqlDataAdapter(selectCommandText, sqlConnection);
				DataSet dataSet3 = new DataSet();
				sqlDataAdapter3.Fill(dataSet3, "tblHR_OfficeStaff");
				dataRow[3] = dataSet3.Tables[0].Rows[0]["EmployeeName"].ToString();
				if (dataSet.Tables[0].Rows[i]["BGGroupId"].ToString() != "1")
				{
					string cmdText3 = fun.select("Symbol AS BGgroup", "BusinessGroup", "Id='" + dataSet.Tables[0].Rows[i]["BGGroupId"].ToString() + "'");
					...
					{
						dataRow[5] = dataSet4.Tables[0].Rows[0]["BGgroup"].ToString();
						dataRow[4] = "NA";
					}
				}
				else
				...
				dataRow[6] = dataSet.Tables[0].Rows[i]["ProjectName"].ToString();
				string cmdText4 = fun.select("CityName", "tblCity", string.Concat("CityId='", dataSet.Tables[0].Rows[i]["PlaceOfTourCity"], "'"));
				SqlCommand selectCommand4 = new SqlCommand(cmdText4, sqlConnection);
				SqlDataAdapter sqlDataAdapter5 = new SqlDataAdapter(selectCommand4);
				DataSet dataSet5 = new DataSet();
				sqlDataAdapter5.Fill(dataSet5);
				string cmdText5 = fun.select("StateName", "tblState", string.Concat("SId='", dataSet.Tables[0].Rows[i]["PlaceOfTourState"], "' "));
				...
				string cmdText6 = fun.select("CountryName", "tblCountry", string.Concat("CId='", dataSet.Tables[0].Rows[i]["PlaceOfTourCountry"], "' "));
				...
				{
					value = dataSet7.Tables[0].Rows[0]["CountryName"].ToString() + ", " + dataSet6.Tables[0].Rows[0]["StateName"].ToString() + ", " + dataSet5.Tables[0].Rows[0]["CityName"].ToString();
				}
				dataRow[7] = value;
				dataRow[8] = fun.FromDateDMY(dataSet.Tables[0].Rows[i]["TourStartDate"].ToString());
				dataRow[9] = fun.FromDateDMY(dataSet.Tables[0].Rows[i]["TourEndDate"].ToString());
				dataRow[10] = dataSet.Tables[0].Rows[i]["TINo"].ToString();
				string cmdText7 = fun.select("TIMId", "tblACC_TourVoucher_Master", "TIMId='" + Convert.ToInt32(dataSet.Tables[0].Rows[i]["Id"].ToString()) + "' AND CompId='" + CompId + "' And FinYearId<='" + FyId + "'");
	...
	protected void GridView2_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		...
		try
		{
			if (e.CommandName == "Del")
			{
				...
				SqlCommand sqlCommand = new SqlCommand(fun.delete("tblACC_TourAdvance", "MId='" + num + "'"), sqlConnection);
				sqlCommand.ExecuteNonQuery();
				SqlCommand sqlCommand2 = new SqlCommand(fun.delete("tblACC_TourAdvance_Details", "MId='" + num + "'"), sqlConnection);
				sqlCommand2.ExecuteNonQuery();
				string cmdText = fun.select("*", "tblACC_TourAdvance_Details", "MId='" + num + "' ");
				SqlCommand selectCommand = new SqlCommand(cmdText, sqlConnection);
				SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(selectCommand);
				DataSet dataSet = new DataSet();
				sqlDataAdapter.Fill(dataSet);
				if (dataSet.Tables[0].Rows.Count == 0)
				{
					SqlCommand sqlCommand3 = new SqlCommand(fun.delete("tblACC_TourIntimation_Master", "Id='" + num + "'"), sqlConnection);
