...
public class Module_SalesDistribution_Transactions_CustEnquiry_Edit : Page, IRequiresSessionState
{
	...
	protected void Page_Load(object sender, EventArgs e)
	{
		try
		{
			...
			string cmdText = fun.delete("tblFile_Attachment", "CompId ='" + CompId + "' AND FinYearId ='" + FinYear + "' AND SessionId = '" + SId + "'");
	...
	public void BindDataCust(string Cid, string EID)
	{
		try
		{
			...
			for (int i = 0; i < dataSet.Tables[0].Rows.Count; i++)
			{
				...
				string cmdText2 = fun.select("CloseOpen", "SD_Cust_WorkOrder_Master", " EnqId='" + dataSet.Tables[0].Rows[i]["EnqId"].ToString() + "'");
