...
public class Module_Design_Transactions_Slido_Gunrail_Details : Page, IRequiresSessionState
{
	...
	protected void btnProceed_Click(object sender, EventArgs e)
	{
		try
		{
			...
			string cmdText = fun.select("*", "tblDG_Gunrail_CrossRail_Temp", "CompId='" + CompId + "' AND SessionId='" + SId + "'");
			SqlCommand selectCommand = new SqlCommand(cmdText, con);
			SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(selectCommand);
			DataSet dataSet = new DataSet();
			sqlDataAdapter.Fill(dataSet);
			string cmdText2 = fun.select("*", "tblDG_Gunrail_LongRail_Temp", "CompId='" + CompId + "' AND SessionId='" + SId + "'");
			...
			if (dataSet.Tables[0].Rows.Count > 0 && dataSet2.Tables[0].Rows.Count > 0 && txtPitch.Text != string.Empty)
			{
				...
				string cmdText3 = fun.insert("tblDG_Gunrail_Pitch_Master", "SysDate,SysTime,SessionId,CompId,FinYearId,Pitch,WONo,Type", "'" + currDate + "','" + currTime + "','" + SId + "','" + CompId + "','" + FinYearId + "','" + num + "','" + WONo + "','" + num2 + "'");
				SqlCommand sqlCommand = new SqlCommand(cmdText3, con);
				con.Open();
				sqlCommand.ExecuteNonQuery();
				con.Close();
				string cmdText4 = fun.select("Id", "tblDG_Gunrail_Pitch_Master", "CompId='" + CompId + "' Order By Id Desc");
				...
				{
					string cmdText5 = fun.insert("tblDG_Gunrail_CrossRail", "MId,Length,No", "'" + text + "','" + Convert.ToDouble(decimal.Parse(dataSet.Tables[0].Rows[i]["Length"].ToString()).ToString("N3")) + "','" + Convert.ToDouble(decimal.Parse(dataSet.Tables[0].Rows[i]["No"].ToString()).ToString("N3")) + "'");
					SqlCommand sqlCommand2 = new SqlCommand(cmdText5, con);
					con.Open();
					sqlCommand2.ExecuteNonQuery();
					con.Close();
				}
				string cmdText6 = fun.delete("tblDG_Gunrail_CrossRail_Temp", "CompId='" + CompId + "' AND SessionId='" + SId + "'");
				...
				{
					string cmdText7 = fun.insert("tblDG_Gunrail_LongRail", "MId,Length,No", "'" + text + "','" + Convert.ToDouble(decimal.Parse(dataSet2.Tables[0].Rows[j]["Length"].ToString()).ToString("N3")) + "','" + Convert.ToDouble(decimal.Parse(dataSet2.Tables[0].Rows[j]["No"].ToString()).ToString("N3")) + "'");
					SqlCommand sqlCommand4 = new SqlCommand(cmdText7, con);
					con.Open();
					sqlCommand4.ExecuteNonQuery();
					con.Close();
				}
				string cmdText8 = fun.delete("tblDG_Gunrail_LongRail_Temp", "CompId='" + CompId + "' AND SessionId='" + SId + "'");
				SqlCommand sqlCommand5 = new SqlCommand(cmdText8, con);
				con.Open();
				sqlCommand5.ExecuteNonQuery();
				con.Close();
				string cmdText9 = fun.select("Length,No,Pitch", "tblDG_Gunrail_LongRail,tblDG_Gunrail_Pitch_Master ", "tblDG_Gunrail_Pitch_Master.WONo='" + WONo + "' And tblDG_Gunrail_Pitch_Master.Id=tblDG_Gunrail_LongRail.MId ");
				...
				string cmdText10 = fun.select("Length,No", "tblDG_Gunrail_CrossRail,tblDG_Gunrail_Pitch_Master ", "tblDG_Gunrail_Pitch_Master.WONo='" + WONo + "'  And tblDG_Gunrail_Pitch_Master.Id=tblDG_Gunrail_CrossRail.MId ");
				...
				{
					int node = Convert.ToInt32(dataSet6.Tables[0].Rows[m][0]);
					fun.CopyGunRailToWO(node, WONo, CompId, SId, FinYearId, 0, bOMCId, num16, num6, num5, num10, num15, num8);
				}
				Page.Response.Redirect(Page.Request.Url.ToString(), endResponse: true);
			}
			else
	...
	{
		base.Response.Redirect("~/Module/Design/Transactions/Slido_Gunrail_WO_Grid.aspx?ModId=3&SubModId=131");
	}
}
