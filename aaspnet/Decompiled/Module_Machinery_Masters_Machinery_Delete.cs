...
public class Module_Machinery_Masters_Machinery_Delete : Page, IRequiresSessionState
{
	...
	public void fillGrid(string sql, GridView GridView2)
	{
		...
		try
		{
			...
			for (int i = 0; i < dataSet.Tables[0].Rows.Count; i++)
			{
				...
				string cmdText2 = fun.select("Location ", " tblMS_Master ", string.Concat(" ItemId='", dataSet.Tables[0].Rows[i]["ItemId"], "'"));
	...
	protected void GridView2_RowCommand(object sender, GridViewCommandEventArgs e)
	{
		try
		{
			...
			if (num != 0)
			{
				string cmdText = fun.delete("tblMS_Process", "MId='" + num + "'");
				SqlCommand sqlCommand = new SqlCommand(cmdText, con);
				con.Open();
				sqlCommand.ExecuteNonQuery();
				con.Close();
				string cmdText2 = fun.delete("tblMS_Spares", "MId='" + num + "'");
				SqlCommand sqlCommand2 = new SqlCommand(cmdText2, con);
				con.Open();
				sqlCommand2.ExecuteNonQuery();
				con.Close();
				string cmdText3 = fun.select("tblMS_Spares.Id,tblMS_Spares.MId,tblMS_Spares.ItemId,tblMS_Spares.Qty", "tblMS_Spares,tblMS_Master", " tblMS_Spares.MId=tblMS_Master.Id And tblMS_Master.Id='" + num + "'");
				SqlCommand selectCommand = new SqlCommand(cmdText3, con);
				SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(selectCommand);
				DataSet dataSet = new DataSet();
				sqlDataAdapter.Fill(dataSet);
				string cmdText4 = fun.select("tblMS_Process.PId,tblMS_Process.Id", "tblMS_Process,tblMS_Master", "tblMS_Master.CompId='" + CompId + "' AND tblMS_Master.SessionId='" + sId + "' And tblMS_Process.MId=tblMS_Master.Id And tblMS_Master.Id='" + num + "' Order By tblMS_Process.Id desc");
				...
				{
					string cmdText5 = fun.delete("tblMS_Master", "Id='" + num + "'");
