<%@ Page Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true" CodeFile="Boughtout_Report_Info_Print.aspx.cs" Inherits="ChallanInfo" Title="ERP" %>
     ...
     <br />
       <br />
        ...
        <asp:Table runat="server" ID="tbl">
         <asp:TableRow>
         <asp:TableCell>
         <asp:label runat="server" ID="lbl" Text="WONO."  Font-Bold="true" Font-Italic="true"></asp:label>
         </asp:TableCell>
         <asp:TableCell>
        
         <asp:DropDownList ID="ddlCountries" runat="server" AutoPostBack="true" DataSourceID="DropDownDataSource"
    DataTextField="WONo" DataValueField="WONo" AppendDataBoundItems="true" OnSelectedIndexChanged="DropDownList1_SelectedIndexChanged"  Width="120%">
    <asp:ListItem Text="Select WONo" Value="" />
</asp:DropDownList>
<br />
<asp:SqlDataSource ID="DropDownDataSource" runat="server" ConnectionString="<%$ ConnectionStrings:LocalSqlServer %>"
    SelectCommand="SELECT DISTINCT [WONo] FROM [tblPM_Project_Hardware_MasterD]"></asp:SqlDataSource>
          ...
          <asp:GridView ID="GridView3" runat="server" DataKeyNames="PRJCTNO" AllowPaging="False" PageSize="10000" 
         AutoGenerateColumns="False" RowStyle-HorizontalAlign ="Center"   Width = "100%" Visible="false" DataSourceID="SqlDataSource2" 
...
<HeaderStyle BackColor="#BDBDBD"></HeaderStyle>
         ...
         <asp:TemplateField HeaderText="Print" HeaderStyle-Font-Bold="true"   HeaderStyle-BackColor="red"  HeaderStyle-ForeColor="black" HeaderStyle-HorizontalAlign="Center">
  <EditItemTemplate>
  <asp:Label runat="server" ID="lbl14" Text='<%# Eval("PRJCTNO") %>'></asp:Label>
  </EditItemTemplate>
   <ItemTemplate>
       <asp:LinkButton ID="View" runat="server" CommandName="Select" CommandArgument='<%# Bind("PRJCTNO") %>'>View</asp:LinkButton>
   </ItemTemplate>
   ...
   <asp:BoundField DataField="SysDate" HeaderText="Gen Date" SortExpression="SysDate" HeaderStyle-BackColor = "#BDBDBD"/>
        ...
        SelectCommand="SELECT distinct PRJCTNO,SysDate,SessionId,WONo FROM tblPM_Project_Hardware_MasterD  Where PRJCTNO NOT LIKE '%1%'" FilterExpression="WONo = '{0}'" >
        
        
         <FilterParameters>
                    <asp:ControlParameter Name="WONo" ControlID="ddlCountries" PropertyName="SelectedValue"  />
                </FilterParameters>
...
<asp:GridView ID="GridView1" runat="server" DataKeyNames="PRJCTNO" 
...
<HeaderStyle BackColor="#BDBDBD"></HeaderStyle>
  ...
  <asp:Label runat="server" ID="lbl14" Text='<%# Eval("PRJCTNO") %>'></asp:Label>
   ...
   <asp:BoundField DataField="SysDate" HeaderText="Gen Date" SortExpression="SysDate" HeaderStyle-BackColor = "#BDBDBD"/>
        ...
        ConnectionString="<%$ ConnectionStrings:LocalSqlServer%>"
        SelectCommand="SELECT distinct PRJCTNO,SysDate,SessionId,WONo FROM tblPM_Project_Hardware_MasterD order by PRJCTNO asc" >
         </asp:SqlDataSource>
 ...
 <asp:LinkButton ID="View" runat="server" CommandName="View" CommandArgument='<%# Bind("PRJCTNO") %>'>View</asp:LinkButton>                        </ItemTemplate>
                    ...
                    <ItemTemplate>
                            <asp:Label ID="lblsysdate" runat="server" Text='<%# Bind("SysDate") %>'></asp:Label>
                        </ItemTemplate>                       
                        <ItemStyle HorizontalAlign="Right" />
                    </asp:TemplateField>
                    
                    
                     <asp:TemplateField Visible="True" HeaderText="EmpName"> 
                    <ItemTemplate>
                    ...
                    <asp:BoundField DataField="SysDate" HeaderText="Date" Visible="false" >
                        <ItemStyle HorizontalAlign="Center" />
                    </asp:BoundField>
                    <asp:BoundField DataField="Time" HeaderText="Time" Visible="false" >
                        <ItemStyle HorizontalAlign="Center" />
                    </asp:BoundField>
                    <asp:BoundField DataField="EmpName" HeaderText="Gen By"  Visible="false">
